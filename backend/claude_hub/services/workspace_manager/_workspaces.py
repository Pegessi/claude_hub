...
def _build_agent_tree_block(
    ...
    return (
        ...
        "Canonical agent guide: docs/AGENT_TREE.md (backend-only; no tree UI yet, "
        "follow-up 487c630c). Hub Task/session execute managed_task children; they "
        "are not required to list/wait/ack the tree.\n\n"
        "Runtime: only executor_kind=managed_task is spawnable (Claude/Codex/Cursor "
        "via executor_config.agent_type). native_subagent and external_job public "
        "spawn return HTTP 422 until real runtimes exist.\n\n"
        "call_id: identical retry MUST reuse the same call_id and the same semantic "
        "payload. Every new operation, payload, or attempt uses a new call_id. "
        "Agent Tree reuse with a different action/target/body is HTTP 400. Hub "
        "report intake conflict is HTTP 409. Unavailable executor "
        "(native_subagent/external_job) is HTTP 422. Invalid managed_task config "
        "(Cursor model override, blank model, env/model conflict, pinned-session "
        "mismatch) is HTTP 400, not 422. "
        "Uncertain mailbox paste is recovered with "
        f"{INTERNAL_API_CURL} -X POST {base_url}/api/workspaces/sessions/"
        "<session_id>/retry-uncertain -H 'Content-Type: application/json' "
        '-d \'{"call_id":"<uncertain-call-id>","reason":"operator retry"}\' '
        "(204; do not invent a tree-level retry).\n\n"
        "Discover / list / cold replay:\n"
        f"   {INTERNAL_API_CURL} '{base_url}/api/agent-tree/runs?workspace_id={workspace_id}'\n"
        f"   {INTERNAL_API_CURL} '{base_url}/api/agent-tree/runs?workspace_id={workspace_id}"
        f"&root_id={root_run_id}'\n"
        f"   {INTERNAL_API_CURL} '{base_url}/api/agent-tree/runs/{root_run_id}/events?"
        "since_sequence=0&subtree=true'\n\n"
        "1. Spawn a managed_task child (response is the child run; save `id` and "
        "`context_ref`). Config-driven spawn sends executor_config and no "
        "session_id. Explicit-session routing (Resident master: session_id is "
        "mandatory) must omit executor_config so Hub derives agent_type/model/"
        "solo_mode/target/env from that session "
        "(ManagedTaskAdapter.config_from_session), or the config MUST exactly "
        "match. Do not hard-code Claude. Do not combine hardcoded Claude "
        "executor_config with an arbitrary existing session_id; "
        "validate_session mismatch (Codex/Cursor or other agent/model/target/"
        "env) is HTTP 400. Schema fields for config-driven spawn: agent_type, "
        "model (Claude/Codex only; Cursor MUST omit executor_config.model "
        "because an explicit model override is rejected — HTTP 400, not 422), "
        "env, solo_mode, target, cwd, remote_profile_id, remote_cwd, "
        "remote_reconnect. No Codex session-picker field exists on "
        "executor_config.\n"
        ...
        "2. Wait for directed subtree events from your ACK cursor:\n"
...
def _build_resident_master_prompt(
    ...
    return (
        ...
        "   Explicit-session routing: session_id is mandatory, so omit "
        "executor_config and let Hub derive it (or the config MUST exactly "
        "match). Do not hard-code Claude. Do not combine hardcoded Claude "
        "executor_config with an arbitrary existing session_id; Codex/Cursor "
        "or other mismatch is HTTP 400. "
        "Follow the Agent Tree block above for executor_config, call_id retry, "
        "HTTP 400/409/422, and uncertain-delivery recovery. "
