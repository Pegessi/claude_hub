...
@pytest.mark.asyncio
async def test_stream_inactivity_timeout_reaps_silent_turn(
    store: AgentStreamStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A turn silent past the inactivity timeout is reaped.

    The model API is streaming, so a healthy turn emits events continuously.
    If no event arrives for STREAM_INACTIVITY_TIMEOUT_S, the stream is dead
    (e.g. the model backend stopped responding). The turn is terminalized
    and the subprocess reaped rather than hanging forever. This is the
    primary hang check — faster than the total-duration cap.
    """
    from claude_hub.services.agent_stream import tailer as tailer_module
    from claude_hub.services.agent_stream.claude_jsonl import ClaudeJsonlAdapter
    ...
    # Keep the subscriber attached (user is watching). Force the
    # last-activity stamp past the (zeroed) inactivity timeout.
    tailer._last_event_at = time.monotonic() - 1.0

    await asyncio.sleep(0.3)
    # The silent turn is terminalized (cancelled) and the subprocess reaped.
    assert transport.turn_in_flight is False
    assert transport.stop_called is True
    assert tailer.is_running() is False
    page = await store.read_since(-1, limit=50)
