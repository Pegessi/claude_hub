...
- docs(audit): frontend response-speed audit — produce `docs/working-logs/2026-07-11-frontend-response-speed-audit.md`, a read-only sweep of bundle/network/reactivity/perceived-responsiveness hotspots on top of develop@9fda1fa (post UI-sweep SL-01..SL-13, post SL-12 ETag/304 and board-payload-slim). Build snapshot: 2 JS chunks (index 67 KB gz, AWV 58 KB gz), no chunk >250 KB gz, runtime deps only vue/pinia/marked/dompurify (terminals are iframed ttyd, no xterm in the Vue bundle). 12 prioritized findings (3 high / 5 med / 4 low) with file:line anchors, proposed fixes, impact/effort/risk, and dispatch ordering. Headline wins: (PR-01) board response ships every report body every 2.5 s even though the Kanban only reads the latest per task — project reports to a lite header map on the backend and fetch full history on demand (analogue of the prior board-payload-slim for reports); (PR-02) `reportsForTask()`/`sessionForTask()` non-memoized O(N·R/S) scans called per task card per poll — mirror the existing `latestReportByTaskId` map pattern with a single-pass `reportsByTaskId`/`sessionByTaskId`; (PR-03) `fetchFeedbackLessons` chained after every board poll with no ETag or in-flight dedup. Zero source edits in this task; doc + CHANGELOG only.
