...
- style(ui): unify agent-status-card roster strip — equal heights, consistent icon box, consolidated action buttons. **Equal heights**: added `flex:1 0 auto` to `.agent-status-card-main` + `margin-top:auto` on `.agent-status-actions` so cards flex-stretch to the resident (tallest) height and the actions row pins to a single bottom line across all cards, eliminating the empty band above Env/Delete on non-resident cards. **Uniform glyph box**: the existing `.btn-icon` 14×14 inline-flex centered box (matching `AgentStatusFloatingPanel`'s `panel-refresh-icon` convention) was made authoritative — added `font-variant-emoji: text` (forces text/outline presentation for ⚙ ▶ ⏸ which default to emoji and render oversized relative to × ↻), `text-align:center`, `overflow:hidden`, tokenized `font-size` to `var(--ch-font-sm)`, and dropped the `vertical-align:-1px` hack (the inline-flex box handles centering). `.agent-status-refresh` (28×28) was switched from `inline-block` to `inline-flex; align/justify:center` so its ↻ glyph sits dead-center (previously offset). **Button consolidation**: folded the four duplicated `height:26px / border-radius:5px / font-size:12px / padding:0 9px / cursor / transition` declarations into the shared `.agent-status-pause, .agent-status-run-now, .agent-status-switch-env, .agent-status-delete` group selector and tokenized gaps/padding (`--ch-space-1`, `--ch-space-2`, `--ch-radius-sm`, `--ch-font-sm`); per-button color/border/background/hover/danger styles are preserved. CSS-only in `AgentWorkspaceView.vue`; template/data/backend unchanged; `@media (max-width:768px)` icon-only 36×36 action buttons remain intact (their `font-size:16px` override is preserved); `@media (max-width:480px)` grid layout unchanged. Playwright before/after screenshots in `artifacts/screenshots/` prove equal heights (both cards 171px, actions at same y=314.5), all six glyphs at uniform 14×14, refresh icon centered (7,7 offset in 28×28), responsive widths 768px and 400px render coherent equal-height strips. `pnpm -C frontend lint` + `pnpm -C frontend build` green.
- chore(ui): tokenize stray border-radius values (audit A3/A5/A6) — close the low-risk tail of the round-2 minimalist-elegance audit. **A5**: add a `--ch-radius-pill: 9999px` token to `App.vue` (in both the base `:root` and the `:root[data-theme='light']` block, matching the existing `--ch-radius-*` duplication convention) and replace all six hardcoded `999px` pill radii — 2 in `App.vue` (theme-switch track + thumb) and 4 in `AgentStatusFloatingPanel.vue` (`.trigger-count`, `.panel-mode-switch strong`, `.agent-cli` kind chip, and the min-width:76px status pill) — with `var(--ch-radius-pill)` (9999px guarantees a pill at any element height; visually identical to 999px for these 18–32px elements). **A6**: replace a raw `border-radius: 4px` on `.agent-config-fields--form .acf-select` in `AgentConfigFields.vue` with `var(--ch-radius-sm)` (5px; 1px sub-pixel change). **A3**: the intentional `border-radius: 2px` on `.layout-cell` in `LayoutSelector.vue` (functional geometry for the tiny layout-preset diagram cells — `--ch-radius-sm`=5px reads too rounded) is LEFT UNCHANGED with a clarifying comment added; zero visual change. **A4 intentionally deferred** as an already-documented zero-risk exception (LoginView card `8px` sits between `--ch-radius-md`=7 and `--ch-radius-lg`=10; redefining the global token risks regressions across every consumer and LoginView is out of scope). Scope: `App.vue` + `AgentStatusFloatingPanel.vue` + `AgentConfigFields.vue` + `LayoutSelector.vue` + this CHANGELOG bullet; no `TabBar.vue`/`AgentWorkspaceView.vue`/`LoginView.vue` edits; `pnpm -C frontend lint` + `pnpm -C frontend build` green.
- feat(ui): smooth modal enter/leave transitions via `<Transition name="modal-fade">` wrapping all parent-owned-`v-if` overlay roots in TabBar and AgentWorkspaceView (audit F2, boundary-safe re-scope). Fifteen modals animated (4 in TabBar: create-tab / file-browser / close-confirm / switch-env; 11 in AWV: task-detail slide-over / markdown preview / image lightbox / create-workspace / resident-agent / new-task / edit-task / lessons / agent-options / agent file-browser / agent switch-env). Motion is token-driven — enter uses `var(--ch-motion-drawer)` (=180ms cubic-bezier(0.2,0,0,1)), leave uses `var(--ch-motion-fast)` (=120ms ease); the overlay fades opacity while the inner dialog/panel/image adds a subtle translateY(4→2px) + scale(0.98→1) rise-in. **EnvPresetManager explicitly descoped** — animating it correctly requires relocating its root `v-if="visible"` (a visibility-contract change interacting with PR-11's `{ immediate: true }` draft-loading watch), which is deferred to a separate visibility-contract refactor. EPM call sites are NOT wrapped; `EnvPresetManager.vue` has zero edits. Reduced-motion users are covered by the existing RM-01 universal net in `App.vue:965-973` (which forces `transition-duration: 0.001ms !important` for `prefers-reduced-motion: reduce`) — no per-component media query added.
- feat(ui): add terminal "Connecting…" overlay during ttyd handshake (audit F1) — show an elegant minimalist overlay (20px spinner + "Connecting…" label) inside the terminal canvas while the ttyd WebSocket is not yet ready, so users see a calm loading state instead of a blank iframe. Keys off the EXISTING per-tab readiness map `getTerminalState().ready[tabId]` (already written by the `terminal-ready` / `terminal-not-ready` postMessage handlers at old L1262–1280), bridged to a Vue `ref()` (`activeTabReady`) via a `syncActiveTabReady()` helper called from the `watch(props.tabId, {immediate:true})` and directly from both message handlers. Overlay is `position:absolute` inside the already-`position:relative` `.terminal-container` (no layout shift), `pointer-events:none` (does not intercept terminal input), and uses existing design tokens (`--ch-color-app-bg`, `--ch-color-accent`, `--ch-color-border-muted`, `--ch-color-text-muted`, `--ch-font-sm`, `--ch-weight-medium`, `--ch-space-3`, `--ch-motion-standard`, `--ch-motion-ease`). Reduced-motion safe: a local `@media (prefers-reduced-motion: reduce)` rule disables the spinner spin and the enter/leave fade transition; the app-wide RM-01 universal guard is a second layer. `<Transition name="terminal-connecting-fade">` fades opacity 0→1 over `var(--ch-motion-standard)` on enter and 1→0 on leave; previously-ready cached tabs set `activeTabReady=true` before first render (via the watch `immediate:true` sync) so the overlay does not flash. SINGLE-WRITER SCOPE: only `frontend/src/components/TerminalView.vue` touched (template overlay, `activeTabReady` ref, sync helper, watch extension, both message-handler assignments, ~45 lines of scoped CSS); `TerminalPane.vue` not edited (overlay is self-contained in the iframe container); `TabBar.vue`, `AgentWorkspaceView.vue`, `App.vue`, store, backend untouched. `pnpm -C frontend lint` green; `pnpm -C frontend build` green (83 modules, 1.12s). `main@2d034f6` untouched.

- feat(ui): terminal connection-failure state with Retry (audit A10) — when ttyd genuinely fails to connect (iframe `@error` for network/load failures, or no `terminal-ready` postMessage within 10 s of iframe insertion), replace the eternal F1 Connecting spinner with a centered, calm "Connection lost" overlay containing one muted helper line and a single Retry button. The F1 Connecting overlay (DOM, classes, transitions, timings, styles, reduced-motion rule) is byte-for-byte preserved and continues to show during the normal ~200–800 ms handshake; the new failure overlay is a sibling `<Transition>` rendered at z-index 6 (above F1's z-index 5) only when `connectionErrors[tabId]` is true. Failure detection: (a) iframe native `@error` event covers backend-down / DNS / 502 cases where the iframe never loads a ttyd document; (b) a connect-timeout (10 s, armed when a tab enters `cachedTabIds`, cleared on `terminal-ready` or tab teardown) covers tmux-crashed / WS-never-opens cases where the iframe loads but ttyd never reaches a ready state — the existing SAB fast-path / pollTerminalReady runs for ~1.8 s, so 10 s gives a comfortable margin and never fires on a slow-but-healthy connect. Failure state lives in `terminalStore` as a per-tab `Record<string, boolean>` (`connectionErrors`) with `markConnectionFailed`/`clearConnectionError`/`isConnectionFailed` actions following the store's existing setup-store pattern, so the flag follows the tab across pane moves and split-layout changes. Retry behavior: clicking Retry clears the error flag, drops `activeTabReady` so the F1 spinner shows again, tears down the dead iframe's ready-state and SAB ring, evicts the tabId from `cachedTabIds` and re-pushes it on the next microtask — Vue recreates the iframe element with a fresh `/api/terminal/proxy/{tabId}/` src, reusing the existing load/SAB injection path with no new backend endpoint and no new transport; the connect-timeout is re-armed so a still-down backend surfaces the same overlay after another 10 s. Successful `terminal-ready` postMessage clears both the pending timeout and any error flag for that tab so the overlay vanishes the moment the terminal is live. Styling is token-only (uses `--ch-color-app-bg`, `--ch-color-text`, `--ch-color-text-muted`, `--ch-color-accent`, `--ch-color-accent-hover`, `--ch-color-text-inverse`, `--ch-color-accent-ring-strong`, `--ch-font-sm`, `--ch-font-base`, `--ch-weight-medium`, `--ch-weight-semibold`, `--ch-space-2`, `--ch-space-3`, `--ch-space-4`, `--ch-space-6`, `--ch-radius-md`, `--ch-motion-fast`), shares F1's `position:absolute` + `inset:0` + `background:var(--ch-color-app-bg)` backdrop treatment, sets `pointer-events:auto` so Retry is clickable, and honors reduced-motion via a local `@media (prefers-reduced-motion: reduce)` block that disables the button transition (the app-wide RM-01 guard is defense-in-depth). No auto-retry loops; no diagnostics panel; no new dependencies or design tokens; no backend/ttyd/tmux changes. Scope: `frontend/src/components/TerminalView.vue` + `frontend/src/stores/terminalStore.ts` + this CHANGELOG bullet only; TabBar.vue / AgentWorkspaceView.vue / LayoutSelector.vue / App.vue / AgentStatusFloatingPanel.vue / AgentConfigFields.vue not touched. `pnpm -C frontend lint` green; `pnpm -C frontend build` green.
- feat(ui): add `BaseModal` + `EmptyState` shared presentational primitives (audit A7, first increment) — two new self-contained, token-driven components under `frontend/src/components/`. `BaseModal.vue` owns the modal chrome consumers currently hand-roll (backdrop, centering, ESC-to-close, backdrop-self-click-to-close, entrance/exit motion, `role="dialog"`/`aria-modal`); slot-based (`header` → falls back to `title` prop, default body, `footer`); v-model-friendly (`open` prop + `update:open` emit, plus a distinct `close` event); enter ~180ms `cubic-bezier(0.2,0,0,1)` / leave ~120ms ease via `--ch-motion-drawer`/`--ch-motion-fast`; a **local** `@media (prefers-reduced-motion: reduce)` guard neutralizes its own motion so it is self-sufficient regardless of the app-level RM-01 guard. `EmptyState.vue` is the canonical "nothing here yet" affordance — slot-based (`icon`, `title`→prop, `description`→prop, `action`), centered, restrained, muted helper text. Every design-token reference in both files carries a literal fallback (`var(--token, <value>)`) so the primitives render correctly on any tree whether or not the `--ch-*` scale is defined. Proof-of-use migration (deliberately ONE safe consumer this increment): `EnvPresetManager.vue`'s editor empty-state swaps its bespoke inner `<p>Select a preset or create a new one.</p>` for `<EmptyState title="…">`, keeping the existing `.env-empty-state` dashed-frame wrapper so the placeholder's frame/fill-height/centering are unchanged (the text now renders via the shared primitive; minor, intentional typographic normalization to the canonical empty-state title style). No other modal is migrated in this increment and no Toast/ToastHost is introduced. Scope: `BaseModal.vue` (new), `EmptyState.vue` (new), `EnvPresetManager.vue` (import + one template line), `CHANGELOG.md`; `pnpm lint` + `pnpm build` green.
- ci(perf): add a build guard (`frontend/scripts/verify-chunk-split.mjs`) that fails CI if the `agent-config` lazy chunk (EnvPresetManager.vue + AgentConfigFields.vue, ~35 KB gz) re-enters the initial shell — implements audit finding A11 (v3, adds Layer 5 static-import detection per reviewer round-2 feedback). PR-14's ~33 KB gz first-paint win depends on both SFCs staying in the on-demand `agent-config-*.js` chunk (loaded only when the user opens a preset-management modal) AND on entry-level code loading them through `defineAsyncComponent` so the chunk is fetched behind a dynamic `import().then()` rather than as an eager static dependency. The guard runs after `pnpm build` and uses four independent content-level signals plus a manifest cross-check: (1) exactly one `dist/assets/agent-config-*.js` lazy chunk exists (catches a deleted/renamed manualChunks rule, which inlines Vue runtime back into index.js and bloats it from ~29 KB gz to ~61 KB gz); (2) the entry chunk does NOT contain `__name:"EnvPresetManager"` or `__name:"AgentConfigFields"` SFC markers while the lazy chunk does contain both (mangling-stable content fingerprint — Vue's SFC compiler bakes these string literals into production output as devtools identifiers); (3) the entry chunk contains AT LEAST 2 `import("./agent-config-HASH.js").then(...)` dynamic expressions (one per async component — catches a revert of either TabBar defineAsyncComponent wrapper back to a static `import`, which leaves the chunk as a separate file but loses the lazy trigger and pulls the component default export into the static binding list); (4) **[v3 / Layer 5]** the agent-config chunk does NOT directly export either SFC's `__scopeId`-wrapped render function — in the healthy split, Vue wraps each SFC in a `_withScopeId(setup, [["__scopeId","data-v-..."]])` helper and only exports a `Object.freeze(Object.defineProperty({__proto__:null,default:wrapped},Symbol.toStringTag,{value:"Module"}))` Module-namespace interop object; when an entry-level module (App.vue, LayoutSelector.vue, …) adds a static `import X from '@/components/EnvPresetManager.vue'` while TabBar's defineAsyncComponent stays intact (the hybrid regression that slipped past v2), Rollup adds a direct export of the wrapped SFC to satisfy the static default binding, and Layer 5 fails loudly with a precise diagnostic naming the leaked component. When built with `--manifest` (bonus layer), it additionally verifies the chunk is flagged `isDynamicEntry: true` and appears in the entry's `dynamicImports`. Naive grep approaches are avoided: bare identifier matches false-positive on the dynamic import glue and __vite__mapDeps CSS asset strings; top-level `import ... from "./agent-config-*.js"` false-positives on Rollup's cross-chunk Vue-runtime re-export facade (present even in the correctly-split build, carrying only helper symbols, not component defaults); counting dynamic `.then()` sites alone is insufficient (v2's blind spot, addressed by Layer 5) because a newly-added static import in a sibling entry module adds a static edge while leaving the existing dynamic sites intact. Added as an OPTIONAL `pnpm verify-chunk-split` script entry and as ONE appended step in `.github/workflows/ci.yml` (after the existing "Run Type check and build" step, no restructuring); the existing build command is unchanged (no --manifest flag), so the manifest layer is a skipped bonus in CI and signals (1)-(4) provide coverage. Validated six ways: (a) passes on current develop build (layers 1-4 all green); (b) FAILS when manualChunks is disabled (chunk missing, clear diagnostic); (c) FAILS when BOTH TabBar defineAsyncComponent calls are reverted to static imports (0 dynamic sites + Layer 5 direct-export leak); (d) FAILS when ONE is reverted (1 site < 2 required + direct-export leak); (e) FAILS when a new static import is added in App.vue while TabBar dynamic stays (hybrid regression — Layer 5 catches the direct SFC export even though Layer 3 still sees 2 dynamic sites); (f) FAILS when the static import is added in LayoutSelector.vue instead (same Layer 5 signal, different component). All probes fully reverted before commit; forbidden files (all .vue components, terminalStore.ts, vite.config.ts manualChunks logic) untouched.

- perf(bundle): defer `agent-config` chunk (EnvPresetManager + AgentConfigFields, ~95 KB raw / 35 KB gz) off the initial shell by completing the TabBar-side async conversion (audit PR-14, realizes PR-11's deferred win) — PR-11 converted the AWV side to `defineAsyncComponent` but TabBar still statically imported both components, so the shared `agent-config` chunk stayed modulepreloaded into the initial shell. Convert TabBar.vue's static `import AgentConfigFields from '@/components/AgentConfigFields.vue'` and `import EnvPresetManager from '@/components/EnvPresetManager.vue'` (old L649-650) to `const X = defineAsyncComponent(() => import('@/components/X.vue'))` matching the existing `AgentStatusFloatingPanel` idiom at L663-665. Gating: (a) `AgentConfigFields` at TabBar L292 already sits inside `v-if="showModal"` (L197 create-tab modal overlay) — no template change needed; async loader fires only when user opens New Tab. (b) `EnvPresetManager` at TabBar L595 had `:visible="showSwitchEnvManager"` but no `v-if` — added `v-if="showSwitchEnvManager"` at the call site (mirroring PR-11's AWV fix at L2943) so the chunk is fetched only on Manage-presets open. EPM's `{ immediate: true }` watch (from PR-11, `EnvPresetManager.vue` L253-276) ensures `draftName`/`draftText` populate on first mount with `visible=true`; the `if(newVisible)` guard keeps this safe for any caller that mounts with `visible=false`. **vite.config.ts**: keep the PR-05 `manualChunks` rule (prevents EPM+ACF duplication across the two async importers) but add `build.modulePreload.resolveDependencies` filter that excludes `agent-config` from `<link rel="modulepreload">` so the JS chunk is truly deferred (fetched only on dynamic import). **Measured build delta** (develop @ b403823 baseline → this branch, `pnpm -C frontend build`): `index-*.js` 94.01 KB raw / 29.62 KB gz → 94.15 KB / 29.68 KB gz (noise, +0.14 KB raw); `agent-config-*.js` 89.73 KB / 34.90 KB gz — **removed from `index.html` modulepreload** (baseline had `<link rel="modulepreload" href="agent-config-*.js">`, post-PR-14 `index.html` contains only `index-*.js` script + stylesheets). `agent-config-*.css` (8.96 KB / 1.71 KB gz) remains as a render-blocking `<link rel="stylesheet">` — Vite's CSS extraction includes it because the scoped styles are referenced by the async modules; 1.71 KB gz is negligible and the CSS is harmless when the components are not mounted (scoped selectors don't match any DOM). **Net initial-shell reduction: ~33 KB gz** (34.9 KB gz JS deferred − 1.71 KB gz CSS still in initial HTML). Behavior unchanged: new-tab form still creates tabs correctly; Switch-Env > Manage-presets modal still opens and populates draft on first open. `pnpm -C frontend lint` + `pnpm -C frontend build` green. Changed files: `frontend/src/components/TabBar.vue` (static imports → async, EPM v-if gating) + `frontend/vite.config.ts` (modulePreload filter, manualChunks comment update) + this CHANGELOG bullet. `main@2d034f6` untouched.

- perf(ui): lazy-load AWV's modal-gated heavy child components via defineAsyncComponent (audit PR-11) — convert AgentWorkspaceView.vue's static imports of `EnvPresetManager` (615 lines, Switch-Env preset manager) and `AgentConfigFields` (336 lines, used inside the resident-agent modal and add-agent modal) to `const X = defineAsyncComponent(() => import('@/components/X.vue'))`, matching the idiom already used for `AgentStatusFloatingPanel` in TabBar.vue:663-665 and for AWV itself in App.vue:146-148. Add `defineAsyncComponent` to the vue import. Gating: ACF at L1940 is already inside `v-if="showResidentAgentModal"` (L1792) and ACF at L2727 inside `v-if="showAgentOptionsModal"` (L2570), so Vue does not instantiate those components — and does not fire the async loader — until the first modal open; added `v-if="showSwitchEnvManager"` to the `<EnvPresetManager>` call site at L2940 (EPM's own root has `v-if="visible"` but that does not prevent instantiation, so the caller v-if is required to delay the dynamic import until the user clicks "Manage presets"). **Kept static (HARD constraints from the audit):** `MarkdownContent` (8 report-rendering sites on the main paint path), `NetworkAccessMenu` (header `variant="menu"` always mounted), `AgentAvatar` (169 lines, agent cards on the paint path), `LoadingButton` (113 lines, pervasively used). **vite.config.ts `manualChunks` 'agent-config' rule from PR-05 is intentionally preserved** — TabBar still imports EPM/ACF statically (TabBar.vue:649-650) for its own Create-Tab form and Switch-Env modal, so those modules live in a synchronous shared `agent-config-*.js` chunk that the index shell preloads at first paint; in the steady state AWV's async imports therefore resolve immediately to the already-loaded module and the AWV chunk shrinks only by the ~0.4 KB of static-import boilerplate. The conversion is still the correct semantic "load on first open" annotation and removes AWV's synchronous import edge so a future TabBar-side task (which would need its own GP/review because ACF in TabBar's Create-Tab modal is v-if="showModal" at L197/L292 — an identical candidate) can move the shared agent-config chunk off the critical path without revisiting AWV. **Follow-up (adversarial review)**: with AWV now gating EPM at the call site with `v-if="showSwitchEnvManager"`, EPM mounts with `visible=true` on first open instead of being mounted at AWV start with `visible=false`; the existing `watch(() => props.visible, …)` in `EnvPresetManager.vue:253` (which populates `draftName`/`draftText` from the selected preset when the modal opens) would not fire on that first mount because there is no false→true transition. Added `{ immediate: true }` to that watch (one line); the internal `if (newVisible)` guard keeps this safe for TabBar which mounts EPM at shell start with `visible=false` (the immediate fire just no-ops). `pnpm -C frontend lint` + `pnpm -C frontend build` green.

- perf(ui): drop AWV's redundant acquire/release of the terminal-store agent-status poll so workspace mode stops depending on the 5 s `/api/tabs/status` poll whose data it doesn't own (audit PR-08 dual-poll desync fix) — `AgentWorkspaceView.vue` `onMounted`/`onUnmounted` previously called `terminalStore.startAgentStatusPolling()` / `stopAgentStatusPolling()`, bumping the terminal store's ref-count alongside TabBar (the real owner) and ASFP (which acquires on terminal-mode mount). TabBar is rendered inside `v-show="mode==='terminal'"` in `App.vue` (NOT `v-if`), so it stays mounted across mode switches and always owns one consumer of the poll for its tab-indicator dots; that means the 5 s poll keeps running and `terminalStore.agentStatuses` keeps populating in workspace mode, so the optional `status_text`/`detail` overlay from `terminalStatusByTabId` continues to flow through the three resolver functions (`agentRuntimeStatus`, `agentRuntimeText`, `agentRuntimeDetail`) without AWV holding a consumer. When the overlay is empty (first paint, poll error, or a future change that unmounts TabBar), those resolvers transparently fall back to `board.sessions` fields (`agent.runtime_status`, `agent.current_task_id` → task title, `agent.workspace_path`) populated by AWV's own authoritative 2.5 s board poll, so agent chips stay correct. Effect: removes an unnecessary ref-count pair that semantically coupled AWV to a poll it doesn't drive and removes the dual-source desync hazard between the 5 s terminal poll and the 2.5 s board poll; no change to terminal-mode behavior (TabBar/ASFP start/stop calls and the store's `statusPollConsumers` arithmetic are byte-identical), no change to `terminalStatusByTabId` or the three resolvers, no backend/store/TabBar/ASFP edits. Scope: `AgentWorkspaceView.vue` (two deletions + short ownership comments) + this CHANGELOG bullet; `pnpm -C frontend lint` + `pnpm -C frontend build` green.

- perf(net): add in-flight promise coalescing + loaded-cache to remote-profiles fetch (audit PR-09) — `AgentWorkspaceView.vue` previously called `fetch('/api/remote/profiles')` directly from a local `fetchRemoteProfiles()` helper, with neither deduplication nor any caching. Three separate call paths could therefore fire concurrent GETs for the same workspace-global resource: opening the workspace create/edit modal (workspace modal setup), opening the add-agent modal (`handleAgentTargetChange`), opening the resident-agent modal (`openResidentAgentModal` — guarded by a `remoteProfiles.length === 0` pre-check but races with any in-flight call), and the two `watch(target === 'remote', ...)` watchers on the workspace form and agent-options form that also call fetch on target toggle. On a first page load that opens the workspace modal AND the resident modal in quick succession (or target-toggles to 'remote' on both forms) the duplicate GETs land back-to-back before the first resolves. Fix: hoist `remoteProfiles`, `remoteProfilesLoading`, and `fetchRemoteProfiles()` into `workspaceStore.ts` using the SINGLETON in-flight Promise pattern that is the natural simplification of the existing keyed `Map<string, Promise<void>>` coalescing used by `boardFetches`/`taskReportFetches`/`taskDetailFetches`/`lessonFetches` (remote profiles are workspace-global — `/api/remote/profiles` has no workspace id in the URL — so a single `let remoteProfilesFetch: Promise<void> | null` replaces the Map). Behavior preserved verbatim: (1) concurrent callers coalesce onto one in-flight GET (the same request Promise is returned to every concurrent caller; at most one fetch in flight at any time), (2) cached early-return when `remoteProfiles.value.length > 0` so modal re-opens and repeated target toggles are O(1) no-ops (no spinner flash, no network), (3) a future-facing `{ force?: true }` option bypasses the cache for any future "refresh profiles" UI (no current caller passes it), (4) `remoteProfilesLoading` is set true before the fetch and cleared in a `finally`, (5) errors route through the store's existing `notifyError` helper (matching the `pushNotification({ type: 'error', autoDismissMs: 10000 })` pattern used elsewhere in the store; note: errors are NOT re-thrown because every caller today is fire-and-forget modal/toggle setup and pre-PR-09 AWV swallowed errors via its own `try/catch`), (6) AWV consumes `remoteProfiles`/`remoteProfilesLoading` via `storeToRefs(workspaceStore)` (added to the existing destructuring block) and keeps only a thin wrapper `fetchRemoteProfiles()` that awaits `workspaceStore.fetchRemoteProfiles()` and THEN applies the FORM-SPECIFIC default-select side effect (`if (!workspaceForm.remote_profile_id && remoteProfiles.value.length > 0) workspaceForm.remote_profile_id = remoteProfiles.value[0].id`) — keeping the store form-agnostic so the resident and agent-options forms continue to manage their own defaults at their existing synchronous default sites (`resetResidentAgentForm`, `handleResidentTargetChange` pre-fetch, `agentOptionsForm.remote_profile_id = workspace.remote_profile_id || remoteProfiles.value[0]?.id || ''`), unchanged. Template bindings (`:disabled="remoteProfilesLoading"`, `v-for="profile in remoteProfiles"`, empty-state "Add profiles in ~/.claude_hub/remote_profiles.json…" hint) and both target-toggle watchers are byte-identical at call sites; the local `remoteProfiles = ref<RemoteProfile[]>([])` / `remoteProfilesLoading = ref(false)` declarations at old L3249–3250 are removed (replaced by the storeToRefs bindings). Scope: `frontend/src/stores/workspaceStore.ts` (add `RemoteProfile` type import, refs, singleton in-flight ref, `fetchRemoteProfiles` action + return export) + `frontend/src/components/AgentWorkspaceView.vue` (drop local refs, extend storeToRefs destructure, thin wrapper with post-await default side-effect) + this CHANGELOG bullet; no backend/other-component/App.vue/type/keyframe changes; no TTL/ETag/304 plumbing (endpoint is config-file driven; the force flag covers future refresh needs). `pnpm -C frontend lint` + `pnpm -C frontend build` green.
- perf(ui): bound the **todo** and **queued** kanban columns with the same 10-card collapse already used for done, so a large backlog stops rendering every card on every 2.5 s board poll (audit PR-10) — previously only the `done` column collapsed (10 most-recent, "Show N older" toggle), while `todo` and `queued` rendered their full lists every poll; with a backlog those columns contain the bulk of cards (buttons, selects, avatars, dependency selects, PR-02 linear scans) and dominate per-poll DOM/re-render cost. Generalize the existing done-column machinery in-place inside `AgentWorkspaceView.vue`: rename `DONE_TASK_COLLAPSE_LIMIT` → shared `COLUMN_COLLAPSE_LIMIT = 10`, replace the single `showAllDoneTasks` boolean with a `showAllColumns: Record<WorkspaceTaskStatus, boolean>` reactive record (mirroring the existing `mobileCollapsedColumns` pattern, initialized `{ todo:false, queued:false, working:false, review:false, done:false }`), add helpers `isCollapsibleColumn()` (Set over `{'todo','queued','done'}`), `toggleColumnShowAll()`, and `columnCollapsedCount()`, and fold `visibleDoneTasks`/`doneTasksTotal`/`doneTasksCollapsedCount`/`tasksForColumn` into a unified `tasksForColumn(status)` that tail-slices the most-recent N (via `all.slice(all.length - LIMIT)`, same insertion-order → most-recent-N semantics the done column already used) for collapsible columns when collapsed, and returns the full list for working/review (which stay uncapped — agent parallelism keeps them small by construction). Rename the button class `.column-done-toggle` → `.column-collapse-toggle` (pure rename; desktop `display:inline-flex` rule and hover styles are byte-identical) so the same affordance class serves all three columns without a misleading "done" name; the template toggle v-if is widened from `column.status === 'done'` to `isCollapsibleColumn(column.status) && taskCountForStatus(column.status) > COLUMN_COLLAPSE_LIMIT`, and `aria-expanded`/labels bind to the per-status `showAllColumns[status]` and `columnCollapsedCount(status)`. **Preserved byte-for-byte:** `taskCountForStatus(status)` still returns the raw `tasksByStatusMap[status].length` (true total), so column-header counts, the top-of-board `{{ taskCountForStatus('queued') }} queued` summary chip, empty-state detection (`taskCountForStatus(column.status) === 0`), and the toggle-threshold gate all continue to reflect the full set even when cards are hidden; `working`/`review` lists pass through uncapped; the `done` column keeps its prior "10 most recent + Show N older / Show recent" behavior (tail slice at 10, same label copy, same threshold); mobile column collapse (`mobileCollapsedColumns`, `isMobileColumnCollapsed`, `toggleMobileColumn`, the mobile Show/Hide button, and the `@media (max-width:520px)` `.column-collapse-button` sizing rules at ~L10473) is untouched and orthogonal. No virtual-scroll/windowing/IntersectionObserver/content-visibility (future option per the audit, out of scope); no change to poll cadence, store (workspaceStore.ts locked by PR-03), tokens, keyframes, motion, RM-01 reduced-motion guard, App.vue, or any file outside `AgentWorkspaceView.vue`. `pnpm build` green; `pnpm exec vue-tsc --noEmit` clean.

- perf(ui): add a motion easing token (`--ch-motion-ease`) + panel duration token (`--ch-motion-panel`), de-duplicate light-theme motion-token redefinitions, and tokenize non-AWV keyframe/transition durations (audit RM-02 parallel-safe subset) — before this change the motion vocabulary in `App.vue` exposed only `--ch-motion-fast` (120ms ease), `--ch-motion-standard` (180ms ease), and `--ch-motion-drawer` (180ms cubic-bezier literal), with the custom ease `cubic-bezier(0.2, 0, 0, 1)` repeated as a raw literal in every panel/menu entrance and geometry-collapse transition, and the 140ms panel-entrance duration had no token at all. Additionally, the `:root[data-theme='light']` block redundantly re-declared `--ch-motion-fast` / `--ch-motion-standard` with byte-identical values to the base `:root` declarations. Fix (all additive / value-preserving; zero duration or easing VALUE changes; zero keyframe or @keyframes changes): (1) add `--ch-motion-ease: cubic-bezier(0.2, 0, 0, 1)` to `App.vue` base `:root` so the custom app-wide ease is a single source of truth; (2) add `--ch-motion-panel: 140ms` as a duration-only token (mirrors the drawer/fast/standard shape — duration bundled with easing where appropriate, or duration-only for `animation:` shorthand composition); (3) refactor `--ch-motion-drawer` to `180ms var(--ch-motion-ease)` so it bundles duration+easing (drawer sites can reference `var(--ch-motion-drawer)` alone without a redundant second easing argument); (4) delete the redundant light-theme `--ch-motion-fast` / `--ch-motion-standard` overrides (identical values, dead cascade); (5) replace literal `180ms cubic-bezier(0.2,0,0,1)` with `var(--ch-motion-standard) var(--ch-motion-ease)` on ASFP status-trigger geometry (height/min-width/padding/gap, 4 props), with `var(--ch-motion-drawer)` on TabBar drawer/collapse geometry (13 props across 4 lines: L1386 max-height/padding/gap/border-color, L1565 height/padding/gap/border-radius, L1609 font-size/max-width, L1889 width/height/border-radius), and literal `140ms cubic-bezier(...)` / `120ms cubic-bezier(...)` in keyframe `animation:` shorthands with `var(--ch-motion-panel) var(--ch-motion-ease)` / `var(--ch-motion-fast) var(--ch-motion-ease)` respectively (ASFP `status-panel-in`, LayoutSelector `layout-menu-in`). Cascade note: when `var(--ch-motion-fast)` (which expands to `120ms ease`) is followed by `var(--ch-motion-ease)` in an `animation:` shorthand, CSS uses the last timing-function specified (the cubic-bezier overrides the keyword `ease`), preserving the computed value exactly. Sites that intentionally use keyword `ease` (TabBar `tab-menu-in` at L1734, TabBar `toast-in` at L2477, and all `--ch-motion-fast`/`--ch-motion-standard` hover/background transitions) are intentionally LEFT on the duration-only tokens (no second `var(--ch-motion-ease)`) so their feel stays on the keyword ease. An 80ms tap-response micro-interaction on `.btn` in TabBar (transform 80ms ease, L2350) is out of scope (unique tap-speed duration, keyword ease, not a panel/keyframe/drawer entrance; introducing a new token for a single use would be scope creep). The RM-01 universal `prefers-reduced-motion` guard is untouched and continues to neutralize these tokens for reduced-motion users. **Explicitly deferred (locked by PR-10):** `AgentWorkspaceView.vue` `ws-toast-in` at L10888 (`animation: ws-toast-in 180ms cubic-bezier(...)`) and two 700ms spinner sites (RM-06 territory) — AWV is a HOT file excluded from this PR. Validation: post-edit grep across the 4 edited files confirms zero `cubic-bezier(...)` or `[1-9]0+ms` literals remain inside `transition:` / `animation:` properties (all routed through tokens); `pnpm exec eslint` on the 4 files clean; `pnpm exec vue-tsc --noEmit` clean; `pnpm build` green (chunk sizes within noise of baseline — index 29.62 KB gz, AWV 58.49 KB gz). Scope: `App.vue` + `AgentStatusFloatingPanel.vue` + `LayoutSelector.vue` + `TabBar.vue` + `CHANGELOG.md`; zero backend/TS/type/token-value changes.

- fix(lint): replace a stray raw NUL byte (`\x00`) in `AgentWorkspaceView.vue:3490` with the JS Unicode escape `\u0000` so ESLint's `vue/no-parsing-error` no longer rejects the file (PR-07 regression) — the `feedbackTaskContentKey` cache-key template literal used a literal 0x00 byte as a delimiter between `task.title` and `task.prompt`, which `vue-eslint-parser` treats as a parse-stopping invalid character. The 6-character source escape `\u0000` produces the same runtime U+0000 character, so the delimiter-collision property (NUL cannot appear in realistic task titles/prompts) and the memo semantics are byte-identical; only the on-disk representation changes to ASCII-clean. `grep -aP '\x00' frontend/src/components/AgentWorkspaceView.vue` now returns empty; `pnpm lint` exits 0. Single source line (`frontend/src/components/AgentWorkspaceView.vue`, `feedbackTaskContentKey` return statement) + this CHANGELOG bullet; no other file touched; `pnpm lint` / `pnpm vue-tsc --noEmit` / `pnpm build` all green.

- perf(net): add in-flight promise coalescing + 30 s cadence throttle with a cross-workspace identity guard to `fetchFeedbackLessons` in `workspaceStore.ts` (audit PR-03) — after every successful board poll the store awaited `fetchFeedbackLessons(workspaceId)`, firing an unconditional `GET /api/workspaces/:id/lessons?limit=50` at the ~2.5 s poll cadence (~24 GETs/minute idle) with neither in-flight dedup nor any caching, even though feedback lessons change on human timescales (create/delete/summarize), not poll-cycle timescales. Backend preflight (per the task's mandatory STOP rule) confirmed `GET /lessons` does NOT currently emit `etag`/`cache-control` (HTTP 200; content-length 11 703; content-type application/json; COOP/COEP/CORP; no etag, no cache-control), so adding If-None-Match/304 plumbing in the frontend would be dead code; the ETag path is deferred to a backend prerequisite. Fix (all confined to `workspaceStore.ts`): (1) add a `lessonFetches = new Map<string, Promise<void>>()` that coalesces concurrent calls for the same workspace to a single in-flight GET (mirrors the existing `boardFetches`/`taskReportFetches`/`taskDetailFetches` pattern in the same store); (2) add a `lastLessonsFetchAt = new Map<string, number>()` plus `LESSONS_REFETCH_INTERVAL_MS = 30_000` constant so a fresh poll within 30 s of a successful lessons fetch for the SAME workspace skips the GET entirely; (3) add a scalar `loadedLessonsWorkspaceId: string | null` that records which workspace the current `feedbackLessons.value` actually belongs to — the throttle only short-circuits when `loadedLessonsWorkspaceId === workspaceId` AND the age check passes, mirroring the identity guard at `fetchBoard:297` (`board.value?.workspace.id === workspaceId` before sending If-None-Match). This fixes a v1/v2 cross-workspace staleness defect where a rapid A→B→A workspace switch within the throttle window could leave B's lessons rendered against A's UI; switching workspaces now forces a fresh fetch regardless of age. (4) Torn-write guard: the requested workspaceId is captured at the top of the request IIFE, and after `await response.json()` the assignment to `feedbackLessons.value` / `loadedLessonsWorkspaceId` is skipped if the active workspace has changed mid-flight, so a late response from a switched-away workspace never overwrites the new workspace's data. (5) Mutation invalidation: `createFeedbackLesson`, `deleteFeedbackLesson`, and `summarizeFeedbackLessons` delete the current workspace's `lastLessonsFetchAt` (and defensive `lessonFetches`) entry BEFORE awaiting `fetchFeedbackLessons()` so post-create/delete/summarize refreshes are forced (not throttled); the existing `fetchBoard()` call inside `summarizeFeedbackLessons` is untouched. (6) On `!workspaceId` (logout/unset), reset BOTH `feedbackLessons.value = []` AND `loadedLessonsWorkspaceId = null`. Shape is byte-identical to callers: `activeFeedbackLessons` continues to filter `feedbackLessons.value` by `status==='active'`; create/delete/summarize still surface errors via `notifyError`. Expected idle wire win: ~24 lessons GETs/minute → ~2/minute for a steady workspace (~92% reduction); first-load and post-mutation refreshes remain immediate; workspace-switch latency is bounded by the poll interval (~2.5 s) rather than 30 s. Scope: `frontend/src/stores/workspaceStore.ts` + `CHANGELOG.md` only; zero backend/type/component edits; `pnpm lint` (store-only) / `pnpm exec vue-tsc --noEmit` / `pnpm build` all green (chunk sizes within ~0.2 KB gz noise of baseline — index 29.62 KB gz, AWV 58.49 KB gz).

- perf(a11y): add a global `prefers-reduced-motion` guard so OS reduce-motion users get a near-motionless UI (RM-01) — before this change the only reduced-motion opt-outs in the frontend were (a) a two-property AWV block disabling the board-skeleton shimmer and one progress-fill, and (b) RM-04's local `.mode-content-fade-enter-active { transition: none !important }` rule in `App.vue`; every other animation/transition app-wide (boot spinner, LoadingButton spinner, TerminalPane pane-history spinner, AWV agent-status/workspace-select spinners, toast entrances, panel/menu entrances, MobileControls `<Transition>`, and all ~77 `var(--ch-motion-*)` transitions) ran at full duration regardless of OS preference — an accessibility gap for users with vestibular sensitivity. Fix: add ONE universal-selector net inside `App.vue`'s existing global (non-scoped) `<style>` block, inside `@media (prefers-reduced-motion: reduce)`: `*, *::before, *::after { animation-duration: 0.001ms !important; animation-iteration-count: 1 !important; transition-duration: 0.001ms !important; scroll-behavior: auto !important; }`. Because `App.vue`'s `<style>` is global (no `scoped`), the `*` selector cascades into every child component (AWV, TabBar, LoadingButton, ASFP, LayoutSelector, TerminalPane, MobileControls, toasts, …) WITHOUT editing them — exactly the single-file additive approach the audit prescribed. `0.001ms` durations collapse animations/transitions to effectively-instant while keeping a single rendered frame (avoids the `display:none`/event-suppression edge cases that `0ms` can trigger on some engines); `animation-iteration-count:1` stops infinite spinner loops after one effectively-instant frame; `scroll-behavior:auto` disables smooth scroll for vestibular safety. The RM-04 `.mode-content-fade-enter-active { transition: none !important }` rule is preserved verbatim inside the same media query (placed after the universal rule so its higher-specificity class selector wins and the intent stays obvious to future readers). AWV's existing reduced-motion block is untouched and remains as a defense-in-depth opt-out. Scope: `App.vue` CSS only (one new block + comment, no template/script/keyframe/token changes); zero edits to any other `.vue`/`.ts`/backend file; zero changes to normal-motion behavior (when `prefers-reduced-motion` is NOT set the UI is byte-identical to baseline); `pnpm build` green.

- perf(ui): memoize the feedback-lesson match summary so the workspace board stops re-tokenizing every lesson × every task on every 2.5 s poll (PR-07) — `feedbackLessonMatchSummary` (`AgentWorkspaceView.vue`) previously called `feedbackLessonScore(lesson, task)` for every (task, lesson) pair, and that function re-ran the tokenizer twice per pair (once on the task's `title + prompt` query, once on the lesson text). At T tasks × L lessons that is **2·T·L** tokenizer passes per poll (T=100 × L=50 ≈ 10,000 passes every 2.5 s) just to render one summary line. Two caching layers, both behavior-preserving: (a) a `feedbackLessonTokenIndex` computed pre-tokenizes each active lesson **once**, recomputing only when `activeFeedbackLessons` changes (a lesson fetch/create/update), never on the board poll; (b) a bounded, non-reactive per-task memo `Map` declared at setup scope (so it survives computed re-runs — a `Map` created inside the computed would be discarded each run and never hit) keyed by `task.id` with an exact `title + prompt` content key, so a task whose content is unchanged reuses its matched-lesson result across polls instead of re-tokenizing its query. The memo is cleared whenever the pre-tokenized lesson index changes and evicts entries for task ids no longer on the board, so it stays bounded to the live task set. Steady-state polls do only `Map` lookups + string-key compares (**~0** tokenizer passes); a cold cycle costs at most L lesson-tokenizations + T query-tokenizations (≤ 150 for T=100 × L=50). The tokenizer (`feedbackTokens`, same `/[a-z0-9_.-]{2,}/g` + CJK Han 2/3-gram rule), `feedbackLessonText`, the intersection score, the `score > 0` threshold, the score-desc sort, and the `slice(0, 6)` cap are all byte-identical; `matchedTaskCount` and the three summary strings are unchanged (verified by an old-vs-new parity harness across 9 scenarios: first poll, repeat poll with fresh object refs, content change, task eviction, lesson-set change, no lessons, no matches, empty board — identical summary strings and matched sets incl. order). Single source file (`frontend/src/components/AgentWorkspaceView.vue`); tokenizer/matching/threshold/sort/slice, the board poll cadence, and the store untouched; `pnpm build` (vue-tsc + vite) green.

- fix(ui): correct `LoadingButton` spinner centering so the spin animation does not fight vertical placement (RM-03 follow-up) — the RM-03 fix positioned the spinner with `top:50%; transform:translateY(-50%)` but the existing `@keyframes loading-button-spin` animates `transform: rotate(360deg)`, which overrides the centering translateY while the animation runs and causes the spinner to sit un-centered / bob vertically. Replace the transform-based centering with `top: calc(50% - 0.5em)` (spinner is 1em tall, so subtract half to center vertically), leaving `transform` entirely to the keyframe. Also drops the now-redundant `transform: none` reset in the `hideContentWhileLoading` override (spinner has no transform in the base state to reset). Single source file (`frontend/src/components/LoadingButton.vue`, CSS-only 2-line net change); template/script/keyframe/call-sites untouched; `pnpm build` green.

- perf(ui): fade-in AgentWorkspaceView on terminal→workspace mode switch (RM-04, reduced-motion safe) — wrap the `<AgentWorkspaceView v-if="mode==='workspace'">` mount in `<Transition name="mode-content-fade">` (`App.vue`) so the async workspace view reveals via a calm opacity fade (`opacity: 0 → 1` over `var(--ch-motion-standard)` = 180 ms ease) instead of popping in blank→full-board on first switch. **Enter-only**: no `.mode-content-fade-leave-active` rule, so AWV unmounts instantly on workspace→terminal. Rationale (reviewer-required): `.app` is `flex-direction:column` with both `.terminal-mode-shell` and `.workspace-view` as `flex:1` children; a leave transition would leave two flex:1 children in flow for 180 ms, stacking 50/50. Enter-only is safe because `v-show="mode==='terminal'"` on `.terminal-mode-shell` and `v-if="mode==='workspace'"` on AWV both read the same reactive `mode` ref and flush in the same render tick — the terminal shell is already `display:none` when AWV is inserted, so AWV is the sole flex:1 child in flow during the entire enter phase. Gated by `@media (prefers-reduced-motion: reduce)` that sets `.mode-content-fade-enter-active { transition: none !important; }` so users with the OS preference get an instant swap (RM-01's global guard is a separate HOT/AWV-touching task; this ships a local guard so no unguarded motion lands). Added an inline `loadingComponent: ModeSwitchPlaceholder` to the existing `defineAsyncComponent` (via an `h()` functional component, no new SFC/file) showing a centered 24px spinner (`.mode-fade-spinner`, reusing the existing `@keyframes spin`) on a `flex:1` canvas during the first chunk fetch, replacing the prior "anchor stays empty until promise resolves" flash; `delay:0` = placeholder appears immediately. Terminal-mode shell `v-show`, mode logic, auth/loading banners, bundling (PR-04), and prefetch (`onIdle` import at ~L398) are all byte-identical. `App.vue` is the only source file touched (template wrap, `h` added to vue import, 3-line functional component, defineAsyncComponent option form, ~45 lines of CSS); no new deps; no new tokens/keyframes; AWV/TabBar/MobileControls/LoadingButton/backend untouched.

- fix(ui): stabilize `LoadingButton.vue` so the label no longer jumps ~1.45em when the button enters `loading` (audit RM-03) — the spinner was previously `v-if`'d into the inline flow only when `loading` flipped true (`display:inline-block; width:1em; margin-right:0.45em`), which inserted ~1.45em of new inline geometry the moment an async action started, pushing the slot content (label + optional leading icon) right on every submit/dispatch/refresh. Keep the spinner element in the DOM at all times and position it `position:absolute` (out of inline flow) in the base state, placed at the inner-left edge of the button's existing leading padding (`left:0; top:50%; transform:translateY(-50%)`), with `opacity:0; pointer-events:none;` when idle so the resting state is **pixel-identical to today** (zero spinner footprint, content at its original position). When `.loading-button--loading` is applied the spinner becomes `opacity:1` and animates (same 700ms linear infinite spin, same 1em × 1em size, same `2px solid currentColor; border-right-color:transparent; border-radius:999px` visual) while the slot content stays in its natural position (zero reflow, zero horizontal shift). The `hideContentWhileLoading` modifier (used by icon-only 28×28 refresh buttons in ASFP and AWV) overrides the spinner back to `position:static; display:inline-block; margin-right:0;` so it centers in the square button as content is hidden — behavior unchanged for those callers. Single source file (`frontend/src/components/LoadingButton.vue`, template −1 line / CSS +18/-9 lines); script block, `aria-busy`/`disabled`/sr-only/`:focus-visible` rules, and the `loading-button-spin` keyframe are byte-identical; zero call-site edits (TabBar, AWV, ASFP, LayoutSelector, LoginView all untouched); `pnpm build` green.

- docs(audit): perceived-responsiveness & motion audit — produce `docs/working-logs/2026-07-12-perceived-responsiveness-motion-audit.md` (RM-01..RM-08), a read-only discovery doc covering how the UI *feels* (loading/empty states, action feedback, layout shift/pop-in, motion vocabulary), filling the gap between the static-visual audit (SL-01..SL-13) and the raw-speed audit (PR-01..PR-12). 8 file-scoped findings (**2 high / 5 med / 1 low**) with file:line anchors, proposed fixes, impact/effort/risk, per-file ownership and HOT flags for AWV. Highest-impact: (RM-01, high/S) `prefers-reduced-motion` is honoured in exactly one place (`AWV:7399`, 2 properties) while every spinner/toast/panel/slide is unguarded — add one app-wide media-gated guard; (RM-02, high/M) the `--ch-motion-fast/standard/drawer` token vocabulary (`App.vue:529-531`, used 77×) is bypassed by hardcoded `@keyframes` durations (120/140/180 ms) and a repeated literal `cubic-bezier(0.2,0,0,1)` with no easing token. Also: LoadingButton spinner shifts its label ~1.45em (RM-03), async workspace view pops in with no transition (RM-04), `working` agents have no motion liveness cue (RM-05), five near-duplicate spinner keyframes at two speeds (RM-06), skeleton pattern is board-only (RM-07), two byte-identical toast keyframes tokenized inconsistently (RM-08). Input-debounce prong confirmed clean (no search-as-you-type). Zero source edits in this task; doc + CHANGELOG only.

- perf(bundle): extract `EnvPresetManager.vue` + `AgentConfigFields.vue` into a shared `agent-config` chunk via Vite `manualChunks` (audit PR-05) — EPM (615 lines) + ACF (336 lines) are statically imported by BOTH TabBar (sync shell/index chunk) and the async `AgentWorkspaceView` chunk (imported via `defineAsyncComponent` in App.vue:138). Because AWV is async, Rollup already hoists the shared EPM/ACF modules into the sync shell chunk at the closest-common-ancestor (so they are NOT duplicated into the AWV chunk at baseline — the audit's "copy in both bundles" was written at pre-PR-04 baseline 9fda1fa before chunk topology shifted); they do however live fully inside the render-blocking `index-*.js` shell chunk even though they only render behind modal/v-if gates (`showSwitchEnvManager`). Add a `build.rollupOptions.output.manualChunks(id)` function in `vite.config.ts` that returns the chunk name `'agent-config'` for module ids matching `/components/EnvPresetManager.vue` or `/components/AgentConfigFields.vue`, forcing both modules (and their scoped CSS) into a single synchronous sibling chunk that both `index-*.js` and `AgentWorkspaceView-*.js` statically import. Single source file changed (`frontend/vite.config.ts`, +19 lines) with a short comment explaining the rule; zero `.vue`/`.ts` edits (the deeper option-1 `defineAsyncComponent` conversion that would defer the modal code until user interaction is blocked on the in-flight PR-01 TabBar/AWV edits and remains future work). Build (develop @ b08d6f0 baseline → this branch): new `agent-config-*.{js,css}` emitted (**89.49 KB / 8.96 KB; 34.82 KB gz JS / 1.71 KB gz CSS**), and grep on dist output confirms unique ACF prose (`"Pick a preset for this launch"`) and unique ACF class tokens (`acf-checkbox-label`) live ONLY in `agent-config-*.js` and are referenced via static import by both `index-*.js` and `AgentWorkspaceView-*.js` (zero inlining into either entry). The initial shell `index` JS shrinks **183.20 KB → 93.84 KB raw (−89.36 KB, −48.8%) / 63.11 KB → 29.56 KB gz (−33.55 KB, −53.2%)**; `index` CSS shrinks 74.64 KB → 65.68 KB (−8.96 KB, EPM/ACF scoped styles move to the shared chunk); `AgentWorkspaceView-*.js` is byte-noise-identical (207.82 KB → 207.86 KB, +40 B hash-overhead) because it already imported EPM/ACF via the shell at baseline. Total JS across all chunks is within noise of baseline (402.97 KB → 403.19 KB, +0.22 KB) — this is a critical-path split, not a byte elimination: the browser still requests the same total bytes on first load, but ~89 KB of modal-only code leaves the render-blocking entry chunk into a parallel synchronous dependency chunk (faster parse/eval on the critical path, better cache granularity across releases). `pnpm build` exits 0; `vue-tsc` clean.

- perf(board): truncate report markdown bodies on the 2.5s board poll + additive `resident_report` scalar (audit PR-01) — backend (`backend/claude_hub/services/workspace_manager/_tmux_queries.py`, `backend/claude_hub/models/schemas.py`): extend the existing `_board_report_projection` (which already strips heavy arrays — `changed_files`/`validation`/`risks`/`acceptance_check`/`evaluation_report`/`review_profiles`/`profile_results`/`artifact_refs`) to preview-truncate `message`/`message_en`/`message_zh` to ~160 Unicode code points + a trailing `…` ellipsis, with ASCII word-boundary walk-back (no mid-word slices) and CJK-safe hard-clipping at the cap (multi-byte glyphs never bisected). Module-level `_preview_board_message` helper, plus two constants (`_BOARD_MESSAGE_PREVIEW_CHARS=160`, `_BOARD_MESSAGE_ELLIPSIS="…"`). Add an additive `resident_report: AgentReport | None = None` field to `WorkspaceBoard` (`schemas.py`), populated in `get_board()` as the latest (by `created_at`) report for `workspace.resident_agent_session_id`, projected through the same truncation/heavy-strip path so the resident heartbeat is surfaced without making the UI scan `reports[]` for `task_id=None`. `reports: List[AgentReport]` keeps its existing type and per-task latest-per-bucket semantics — message bodies are just shorter; the on-demand `/api/workspaces/{ws}/tasks/{task}/reports` endpoint (full history, detail panel) is untouched and continues to return full untrimmed markdown bodies and heavy arrays. Frontend: extend `WorkspaceBoard` type with optional `resident_report?: AgentReport | null` (`frontend/src/types/index.ts`), add `residentReport: ComputedRef<AgentReport | null>` to `workspaceStore` (sourced directly from `board.resident_report`, O(1)), export it from the store return, and refactor `AgentWorkspaceView.vue`'s `latestResidentReport` (previously `workspaceStore.reports.filter(report => report.session_id === residentSessionId).sort(byCreatedAt)[0]`, O(R) per poll) to delegate to `workspaceStore.residentReport`. `reports[]` still feeds the existing PR-02 memo maps (`latestReportByTaskId`/`sessionByTaskId`/`reportsByTaskId`/`latestReviewReportByTaskId`) unchanged; cards render byte-identically because `.latest-report` CSS already `-webkit-line-clamp:2`s the preview to ~2 lines at `--ch-font-xs:11px`. ETag path unchanged (`_board_etag` hashes `model_dump` over the full board and automatically picks up the new field). Tests: update `test_board_report_projection_strips_heavy_keeps_card_fields` to assert short messages pass through verbatim; add `test_board_report_projection_truncates_long_messages` (ASCII walk-back, CJK hard-clip at cap + ellipsis, None/empty passthrough), `test_preview_board_message_helper` (None/empty/short/exact-cap/long-boundary cases), and `test_get_board_resident_report_{populated_for_resident_session,none_when_no_resident}` (populates `resident_report`, projects through preview truncation, strips heavy fields, returns null when workspace has no resident session). Validation: `black`/`isort`/`mypy` clean; targeted projection tests pass; `pnpm lint` + `pnpm build` green (shell chunk 183.20 KB / 63.11 KB gz, AWV async chunk 207.71 KB / 58.16 KB gz — unchanged within noise); synthetic 200-long-report + 1-resident microbenchmark shows report message bytes drop from ~822 KB to ~151 KB (**−81.6%**); real-workspace baseline on main was 3.09 MB on wire / 261 K message chars across 200 reports, the same payload post-truncation is projected to ~600–700 KB on wire. Scope: `schemas.py`, `_tmux_queries.py`, `test_workspaces.py`, `types/index.ts`, `workspaceStore.ts`, `AgentWorkspaceView.vue`, `CHANGELOG.md`; `TabBar.vue`/`MarkdownContent.vue`/PR-02 memo internals/on-demand reports endpoint untouched.

- perf(bundle): lazy-load `AgentStatusFloatingPanel.vue` out of the initial shell chunk (audit PR-04) — convert TabBar's static `import AgentStatusFloatingPanel from …` to `defineAsyncComponent(() => import('@/components/AgentStatusFloatingPanel.vue'))` so the 1012-line panel (agent/reviewer list, per-row actions, refresh, resize, observer) ships in an on-demand chunk instead of `index-*.js`. Because TabBar's own `.tab-indicator` status dots read `agentStatuses` and previously free-rode on ASFP starting the poll, agent-status polling ownership moves into TabBar's own `onMounted` (`store.startAgentStatusPolling()`) / `onUnmounted` (`store.stopAgentStatusPolling()`) — so the 5 s poll (and the dots) run even if the user never opens a status chip and the ASFP chunk is never fetched. The poll is ref-counted with a single shared timer (`terminalStore.ts:253-268`), so ASFP acquiring its own consumer when opened is harmless (no second interval, no consumer leak); ASFP itself is left byte-identical. Workspace-mode polling (AgentWorkspaceView's own start/stop pair) is unaffected. Build (develop baseline → this branch): initial `index` JS **194.52 KB → 183.20 KB raw (−11.32 KB) / 67.42 → 63.12 KB gz (−4.30 KB)**, and ASFP's scoped CSS (~11.8 KB) also leaves the shell into the async chunk; new `AgentStatusFloatingPanel-*.{js,css}` (7.31 KB / 11.78 KB) fetched on demand, plus a shared `AgentAvatar-*.js` (~4.6 KB) that Vite now extracts since ASFP + the already-lazy AgentWorkspaceView are both async. Single source file changed (`frontend/src/components/TabBar.vue`, +16/−2); no template/markup change; `pnpm build` exits 0, `pnpm lint` clean.

- perf(md): cache `MarkdownContent` parse/sanitize by raw input string (audit PR-12) — add a module-level bounded (200-entry FIFO) `Map<string,string>` cache in `frontend/src/components/MarkdownContent.vue` keyed by `(raw props.text, linkMarkdownPaths)` so repeated identical markdown strings across the ~8 MarkdownContent instances per task-detail panel (latest-run message, review reason, risks, validation, per-report message/message_en/message_zh) share the already-rendered sanitized HTML. Cache hit path is a single `Map.get()` return with zero calls to `marked.parse` / `DOMPurify.sanitize` / `linkPathMentions`; miss path runs `marked.parse(source, {async:false, breaks:true, gfm:true})` → `DOMPurify.sanitize(html)` → optional `linkPathMentions(sanitized)` (verbatim the prior `safeHtml` body), stores the result, and evicts the oldest entry when the cache exceeds 200. Empty / whitespace-only `props.text` short-circuits to `''` before key construction and never enters the cache. Marked options extracted to a module-level `MARKED_OPTS` constant (values identical — `{async:false as const, breaks:true, gfm:true}`); DOMPurify/linkPathMentions/handleClick/markdownPathPattern bodies byte-identical; `<template>` and `<style scoped>` byte-identical (postcss `*/`-in-comment trap has no surface — no comments added). CSS-only sibling fixes (v-memo on AWV render sites, PR-01 report reference stabilization) are separate PRs and remain out of scope here. Script-only change; ~20 added lines; no new deps; no backend/AWV/other-component touches.

- perf(store): memoize per-task session/report lookup maps in `workspaceStore.ts` (audit PR-02) — the flagship response-speed win for the workspace board. The store already memoized `latestReportByTaskId` as a `computed<Record>` after each board replacement; this mirrors that pattern with three more single-pass maps built O(N+S+R) once per poll instead of re-scanning per call: `sessionByTaskId` (task_id → ManagedSession | null, indexed off a sessionById helper so `sessionForTask(task)` drops the O(S) `sessions.value.find(...)`), `reportsByTaskId` (task_id → AgentReport[] in insertion order so `reportsForTask(task)` drops the O(R) `reports.value.filter(...)`), and `latestReviewReportByTaskId` (task_id → last `review_*` report, derived in the same O(R) pass as `reportsByTaskId`). Existing methods `sessionForTask`/`reportsForTask`/`latestReportForTask` are kept as thin O(1) wrappers so external callers (AWV selectedTask L3547 etc.) keep working with unchanged signatures; a new `latestReviewReportForTask(task)` method is added and AWV's local double-scan wrapper (`workspaceStore.reportsForTask(task).filter(review_*)[last] || null`) is collapsed to a one-line delegation. Per-card v-for render complexity drops from O(N·(R+S)) to O(N+R+S); no behavioral or visual change (identical null / empty-array / insertion-order semantics); ~45 lines added / ~12 lines removed across `workspaceStore.ts` and `AgentWorkspaceView.vue`; TypeScript template byte-identical.

- docs(audit): frontend response-speed audit — produce `docs/working-logs/2026-07-11-frontend-response-speed-audit.md`, a read-only sweep of bundle/network/reactivity/perceived-responsiveness hotspots on top of develop@9fda1fa (post UI-sweep SL-01..SL-13, post SL-12 ETag/304 and board-payload-slim). Build snapshot: 2 JS chunks (index 67 KB gz, AWV 58 KB gz), no chunk >250 KB gz, runtime deps only vue/pinia/marked/dompurify (terminals are iframed ttyd, no xterm in the Vue bundle). 12 prioritized findings (**2 high / 4 med / 6 low**) with file:line anchors, proposed fixes, impact/effort/risk, and dispatch ordering. Highest-impact wins: (PR-01, high/M) board response ships every report body every 2.5 s even though the Kanban only reads the latest per task — project reports to a lite header map on the backend and fetch full history on demand (analogue of the prior board-payload-slim for reports); (PR-02, high/S) `reportsForTask()`/`sessionForTask()` non-memoized O(N·R/S) scans called per task card per poll — mirror the existing `latestReportByTaskId` map pattern with a single-pass `reportsByTaskId`/`sessionByTaskId`. Top of the medium tier (smallest effort → ship next): (PR-03, med/S) `fetchFeedbackLessons` chained after every board poll with no ETag or in-flight dedup (mirror existing boardETags / lastAgentStatusETag pattern); PR-04 (med/S, ASFP lazy-load) and PR-05 (med/S, EPM+ACF dedup-chunk) are shell-chunk wins. Zero source edits in this task; doc + CHANGELOG only.

- style(ui): promote Codex CLI teal `#10a37f` to semantic design token `--ch-agent-codex-fg-cli` (audit SL-13, final second-look close-out) — add `--ch-agent-codex-fg-cli: #10a37f;` to BOTH `App.vue` theme blocks (dark `:root` at L587, `:root[data-theme='light']` at L688), then swap the two remaining hardcoded `color: #10a37f;` CLI-chip sites to `color: var(--ch-agent-codex-fg-cli);`: `AgentStatusFloatingPanel.vue` `.agent-cli[data-kind='codex']` L909 and `AgentWorkspaceView.vue` `.agent-status-cli[data-kind='codex']` L6661. Distinct from the existing `--ch-agent-codex-{bg,fg}` avatar tokens (black/white) which are untouched (avatar palette vs CLI-chip foreground are separate concerns). Both `rgba(16,163,127,0.18)` chip backgrounds preserved as-is (sibling claude/terminal/cursor chips keep their rgba per SL-02/SL-03 precedent; no color-mix, no new postcss dependency). Same `#10a37f` value in dark+light (Codex brand constant; adequate contrast on the 0.18-opacity chip in both themes); zero light-specific variant introduced. CSS-only; template+script byte-identical; no comments added (postcss `*/`-in-comment trap has no surface). 2 new token declarations + 2 one-line consumer substitutions = 4 changed CSS lines across 3 source files. Closes the second-look audit backlog.

- style(ui): snap `AgentWorkspaceView.vue` residual `margin-top: 3/4/6px` literals to `var(--ch-space-1/2)` (audit SL-10, rhythm subset) — narrow 6-site rhythm normalization landing remaining small-px margin-tops on the existing 4/8 space scale defined in App.vue `:root` (`--ch-space-1: 4px` L538, `--ch-space-2: 8px` L539). Three sites are lossless 4px→space-1 swaps (zero layout change): `.workspace-mobile-menu-item--mode + …` L6289, `.workspace-mobile-menu-item--theme` L6314, `.fact-grid strong` L8436. Two sites bucket 3px→space-1 (+1px, intentional grid alignment): `.agent-status-detail` L6676 (xs meta line under the agent CLI chip) and the mobile-viewport `.agent-status-pill` L10353 (grid-column:2 status chip). One site buckets 6px→space-2 (+2px, intentional grid alignment): `.agent-status-meta` L6685 (meta chip row under the status chip). All other `margin-top` literals in AWV are explicitly out of scope and preserved as-is: the `-7px` offset, 0, 1px, 2px, 8px (6 sites), 10px (8), 12px, 14px, 16px, 18px (2), and 20px values all remain literal px. **Zero padding declarations modified**; critically, the adjacent `padding: 3px 6px;` on the mobile `.agent-status-pill` rule (L10354, immediately after the L10353 margin-top edit) is byte-identical pre/post, as is the other `padding: 3px 6px;` at L8945. Template+script byte-identical (lines 1..6145 sha unchanged); no new tokens; no comments added (postcss `*/`-in-comment trap has no surface). 6 one-line CSS edits; CHANGELOG-only otherwise. File-disjoint from prior SL-01/SL-09 (font-weight/font-size) AWV passes that already shipped to develop.

- a11y(ui): add `:focus-visible` keyboard-focus rings to button long tail in `TabBar.vue`, `MobileControls.vue`, and `AgentStatusFloatingPanel.vue` (audit SL-08) — continue the SL-04 pattern: keyboard-Tab focus gets a visible 2px accent ring, pointer click does not. Three consolidated `:focus-visible` blocks (one per file), all using the existing `outline: 2px solid var(--ch-color-accent-ring-strong); outline-offset: 2px;` idiom with zero new tokens, zero template/script changes, zero comments added (postcss `*/` trap has no surface). TabBar covers 11 button classes: `.tab-menu-trigger`, `.tab-close`, `.add-tab`, `.mobile-app-menu-trigger`, `.mobile-app-menu-item`, `.tab-menu-item`, `.path-nav-btn`, `.segment-button`, `.cwd-dropdown-btn`, `.btn` (covers all modal primary/secondary/danger/small/env-manage buttons), `.toast__close`; the SL-05 input/select rules (`.current-path-input`, `.form-group input/select`, `.tab-name-input`) untouched. MobileControls covers `.control-btn` (base class for all 16 key buttons — Esc/Tab/arrows/Enter/Ctrl/Shift/shortcuts; variants `.control-btn-enter`/`.control-btn-shortcut`/`.control-btn-arrow`/`.control-btn-wide` inherit via the base class) and `.toggle-btn` (circular keyboard toggle); existing `.control-btn.active` Ctrl/Shift latch box-shadow ring is preserved (outline renders outside it, two rings = latched + focused). AgentStatusFloatingPanel covers `.status-trigger` (panel open/close chip) and `.agent-row` (agent/reviewer list-row selectors); the already-correct `.panel-mode-switch button:focus-visible` at L766 is left as-is. Explicitly skipped (out of SL-08 scope): `.panel-refresh` (LoadingButton component already applies `.loading-button:focus-visible` with identical idiom); `.resize-handle` (pointer-only drag grip with no hover styling and no keyboard activation); `.tab` and `.file-item` in TabBar (plain `<div>` without `tabindex`/`role=button` — not keyboard-reachable today; adding `tabindex=0` + Enter/Space handlers is a separate remediation). 15 new selectors across 3 rules, 4-line block each, additive only.

- style(ui): snap `AgentWorkspaceView.vue` residual `font-size: 11px` literals to `var(--ch-font-xs)` (audit SL-09) — LOSSLESS token swap: `--ch-font-xs: 11px` exactly at App.vue L548, so computed pixel size is identical across the board (zero visual change, zero layout shift, zero clipping risk). Per-site audit (selector-by-selector) classified all 33 occurrences as semantically extra-small (chips, pills, badges, meta lines, eyebrows, supporting labels, compact 30px action buttons, clamped muted descriptions, status/artifact/attachment/fact-grid metadata, mobile meta subtitles, and the max-width:760px mobile switch-label crunch) — 0 sites intentionally skipped. 33 one-line CSS substitutions (`font-size: 11px;` → `font-size: var(--ch-font-xs);`); no other properties touched; no 10px/12px/13px/14px literals modified (10px was already tokenized to --ch-font-xs in round-1 finding #15); template+script byte-identical; no comments added (postcss `*/`-in-comment trap has no surface). File-disjoint from concurrent SL-05 (EPM/TabBar) and SL-06 (App.vue token) tasks.

- style(ui): add themed `--ch-color-on-warning` token for `.auth-error-banner__retry` CTA legibility (audit SL-06) — producer+consumer fix in one App.vue-only change. Add a new themed design token in both theme blocks, placed with the existing `--ch-color-warning*` group (dark `:root` L503: `--ch-color-on-warning: #1a1a1a;`; light `:root[data-theme='light']` L635: `--ch-color-on-warning: #fdfdfc;`) and switch the retry CTA from hardcoded `color: #1a1a1a;` to `color: var(--ch-color-on-warning);`. Dark theme stays byte-identical (#1a1a1a on #facc15 yellow unchanged); light theme flips from dark `#1a1a1a` on the warm-brown `--ch-color-warning: #906018` background (which read muted/inactive) to near-white `#fdfdfc` for a clearly legible CTA. Hover (`--ch-color-warning-strong`), border (`--ch-color-warning-strong`), and the SL-04 `:focus-visible` outline rule are untouched; `.auth-error-banner__close` (`color: inherit`), banner background/border, and `--ch-color-app-bg: #1a1a1a` canvas token (L465) are all untouched per narrow scope. Three one-line CSS edits (2 token defs + 1 consumer swap); template+script byte-identical; no new comments added so the postcss `*/`-in-comment trap does not apply; no other files touched (file-disjoint from the concurrent ASFP+AWV and EPM+TabBar tasks).

- a11y(ui): unify input focus rings in `EnvPresetManager.vue` + `TabBar.vue` to `:focus-visible` accent ring (audit SL-05) — replace the low-contrast `:focus { outline: none; border-color: var(--ch-color-accent); }` treatment (which missed keyboard-vs-pointer distinction and used ~1.3:1 border contrast) with the 2px accent ring already used by AgentConfigFields and AWV modal inputs. 5 selector edits: (1) `.form-group input:focus` → `.form-group input:focus-visible` in EPM (border-color→`--ch-color-accent-ring-strong`, add `box-shadow: 0 0 0 2px var(--ch-color-accent-ring)`); (2) `.env-textarea:focus` → `.env-textarea:focus-visible` in EPM (same transformation); (3) `.current-path-input:focus` → `.current-path-input:focus-visible` in TabBar (same); (4) `.form-group input:focus, .form-group select:focus` combined selector → same with `:focus-visible` on both in TabBar (same declarations, 4-line rule); (5) `.tab-name-input` inline-rename field: bump the base-rule resting outline from `1px solid var(--ch-color-accent)` to `2px solid var(--ch-color-accent-ring-strong)` for parity. Rationale for keeping `.tab-name-input` on the base rule (no separate `:focus-visible` rule): it is `background: transparent; border: none;` and the outline acts as the field's visible border; rename is programatically focused on appearance (triggered by both keyboard and click), so a `:focus-visible`-only rule would leave it borderless when click-triggered — the spec explicitly allows "outline bump alone" here. Keyboard Tab shows the ring; plain mouse click does not (that is the `:focus-visible` contract) on all 4 `:focus-visible` rules; both dark and light themes render the ring correctly (`--ch-color-accent-ring-strong` has themed definitions at App.vue L495/L626, `--ch-color-accent-ring` at L494/L625). Template+script byte-identical in both files; no postcss `*/`-in-comment trap (no comments added); hover/active/disabled/resting states untouched; no new tokens.

- style(ui): wire claude/terminal chip + origin/autonomy badge hex colors to tokens in `AgentStatusFloatingPanel.vue` and `AgentWorkspaceView.vue` (audit SL-02 + SL-03) — symmetric consumer-half wiring to brand/success tokens defined in App.vue `:root` from 0f036c6. ASFP (2 edits): `.agent-cli[data-kind='claude']` `color: #d97757` → `var(--ch-agent-claude-fg)` (lossless — token holds `#d97757`, zero visual change); `.agent-cli[data-kind='terminal']` `color: #7ee787` → `var(--ch-color-success)` (intentional small hue shift to align with tab-indicator success family: `#4ade80` dark / `#36734d` light). AWV (5 edits): symmetric claude + terminal chip substitutions, plus `.agent-status-master-badge` and `.origin-badge` `color: #c4b5fd` → `var(--ch-agent-origin-bg)` (lossless, token `#c4b5fd`), and `.autonomy-badge` `color: #5eead4` → `var(--ch-agent-autonomy-bg)` (lossless, token `#5eead4`). Codex (`#10a37f`) and cursor (already `var(--ch-color-text)`) untouched per scope (SL-13 later pass). All chip/badge background and border `rgba(...)` literals preserved as-is (no `color-mix`, no new postcss dependency). 7 one-line CSS substitutions; template+script byte-identical in both files (ASFP lines 1..448 sha `2dc8d700…`, AWV lines 1..6145 sha `59986cff…` unchanged); postcss trap avoided; App.vue intentionally not edited (file-disjoint from sibling SL-04 commit 9f8a910 for conflict-free parallel ff-merge).

- perf(net): add ETag/304 to `GET /api/tabs/status` + `If-None-Match` in terminalStore (audit SL-12) — backend (`backend/claude_hub/api/tabs.py`): add module-level `_tab_status_etag(statuses)` helper that dumps via `model_dump(mode="json")`, strips the volatile `sampled_at` timestamp (regenerated every call because `_STATUS_CACHE_TTL_SECONDS=0.75` < 5s poll interval, so it churns even when user-visible content is stable), sorts by `tab_id` for order-independence, canonicalizes with `sort_keys`/tight separators, sha256→32-hex→double-quoted per RFC 7232 (mirroring `workspaces._board_etag` exactly); the handler sets `ETag` + `Cache-Control: no-cache` on every response and returns `Response(status_code=304, …)` when `If-None-Match` matches, bodyless. Frontend (`frontend/src/stores/terminalStore.ts`): add `lastAgentStatusETag` state, send `If-None-Match` header, return early on HTTP 304 (before the `!response.ok` throw, since 304 is not `response.ok`), capture the new `etag` from 200 responses before proceeding with the existing `statusesEqual` / `knownTabIds` flow (which is kept as defense-in-depth for proxies that strip conditional headers). `STATUS_POLL_INTERVAL_MS`, the consumer ref-count model, and reactive state updates are unchanged; polling interval is unchanged. New test `test_tabs_status_etag_returns_304_when_unchanged` in `backend/tests/test_tabs.py` covers 200+ETag, 304 on match, 200 on stale etag, and ETag rotation when status content changes (sampled_at churn is exercised via fresh `datetime.now()` per fake call). No UI/behavior change; idle 5s polls now save both the JSON parse and the ~1-2KB response body on loopback.

- style(ui): cap `AgentWorkspaceView.vue` residual `font-weight: 700` to `--ch-weight-semibold` (audit SL-01) — bulk-replace 24 of the remaining 25 `font-weight: 700;` declarations with `var(--ch-weight-semibold)` (=600, continuing the calm-hierarchy cap already applied to App.vue shell chrome in 430d277); EXCEPTION: the `.agent-status-cli` uppercase xs chip (L6647) uses `var(--ch-weight-medium)` (=500) for cross-component parity with its AgentStatusFloatingPanel twin. Headings, buttons, labels, badges, strong tags, mobile chrome, and the toast icon all soften from bold to semibold; existing 400/500/600 literals and already-tokenized weights are untouched per scope. 25 one-line CSS substitutions; template+script byte-identical; postcss trap avoided.

- a11y(ui): add `:focus-visible` keyboard-focus rings to App.vue shell chrome (audit SL-04) — add additive `:focus-visible` rules for `.auth-error-banner__retry`, `.auth-error-banner__close`, `.mode-button`, and `.theme-switch`, each using `outline: 2px solid var(--ch-color-accent-ring-strong); outline-offset: 2px;` matching the existing AWV/LoadingButton/etc. focus-ring treatment. The theme-switch ring targets the focusable `<button role="switch">` itself (labels and thumb are non-focusable `<span>` children, per actual markup). Existing `:hover` / `.active` / color / size / spacing rules untouched; template+script byte-identical; keyboard Tab shows a visible 2px outline on shell controls; pointer click does not (that is the point of `:focus-visible`); rings render in both dark and light themes (the token has themed definitions at L495 dark / L626 light). 4 additive 3-line CSS blocks (20 added lines total); no comments added so the postcss `*/`-in-comment trap does not apply.

- style(ui): bump `TerminalView.vue` iframe-injected `.xterm` padding from 6px to 8px (audit finding #10) — single-digit literal change in the JS-injected CSS string that styles the terminal iframe document (iframe has no access to App.vue `:root` tokens; literal px is correct). Adds 2px of breathing room between the terminal chrome edge and the text canvas. Validated live at both large (1400×900) and small (380×300) pane sizes against the running :8173 backend via Playwright: `.xterm-screen` fits cleanly inside the `.xterm` padding-box at both sizes (no row/column clipping), `.xterm-viewport` remains absolutely-positioned at inset:0 with overflowY:scroll, `.xterm-scroll-area` stays within the padding box, background fills edge-to-edge with no seam, console errors = 0, and an A/B toggle between 6px and 8px shows a marginally calmer edge without losing a row or column at the tested sizes. Single-character source diff (`6`→`8`); JS string concatenation/quotes/braces/semicolons/!important untouched; zero other rules changed.

- docs(ui): second-look minimalist UI + response-speed sweep (round 2) — produce `docs/working-logs/2026-07-10-minimalist-ui-second-look.md`, a read-only audit against develop@3ee1ef3 after the round-1 minimalist-audit doc had 13 findings shipped across 9 commits. 13 new findings (12 UI, 1 perf; 0 high, 6 medium, 7 low): AWV residual `font-weight:700` cap to semibold (SL-01, 25 sites), App.vue shell-chrome `:focus-visible` gaps (SL-04), input focus-ring inconsistency across EPM/TabBar (SL-05), auth-banner retry hardcoded #1a1a1a against warning bg (SL-06), focus-visible long tail on TabBar/MobileControls/ASFP (SL-08), AWV `font-size:11px` literals (SL-09, 33 sites), small-px rhythm literals (SL-10), terminalStatus ETag/304 (SL-12, perf), plus brand-chip consumer wiring gaps for claude/origin/autonomy (SL-02/SL-03/SL-13). Re-confirms zero `.bak` files; `router/` empty; `main.ts` trivial (17-line bootstrap); all `v-for` have `:key`; polling/SAB fast path sound. Section 3 dispatches exactly 3 bounded, zero-cross-dependency tasks (AWV fw cap, App shell focus-visible, ASFP+AWV brand-wiring) with postcss guard. Read-only doc; zero source edits.

- style(ui): wire `AgentAvatar.vue` Claude/Codex brand hex to App.vue brand tokens (audit finding #2, consumer half) — `.agent-avatar--claude` `background: #f1eee5; color: #d97757;` → `var(--ch-agent-claude-bg)` / `var(--ch-agent-claude-fg)`; `.agent-avatar--codex` `background: #000; color: #fff;` → `var(--ch-agent-codex-bg)` / `var(--ch-agent-codex-fg)`. Tokens were defined in App.vue `:root` in the producer-half commit (0f036c6); each token's value is exactly the hex it replaces, so zero visual change. Update the file's doc-comment "brand marks preserved as flat hex" note to reflect the wiring. Cursor (raised/text) and terminal (sunken/success) variants untouched (already on palette tokens from cde4352). 4 one-line CSS substitutions + 3-line doc-comment update; template+script byte-identical; postcss trap avoided.

- style(ui): snap `AgentWorkspaceView.vue` board shadows/motion/type/rhythm/radii to design tokens (audit findings #12/#13/#14c/#15/#16/#17) — lightbox `box-shadow: 0 12px 48px …` → `var(--ch-shadow-dialog)` (#12); drop the `.agent-status-card:hover` `translateY(-1px)` lift and `0 8px 24px` glow while keeping the border/background hover (the mobile `transform:none;box-shadow:none` override preserved as harmless no-op; `.task-card:hover` out of scope) (#13); snap 9 transition declarations (24 per-property durations) to `--ch-motion-fast`/`--ch-motion-standard`, leaving the 700ms spinners and 180ms cubic-bezier toast entrance untouched (#14c); bulk-replace 25 `font-size: 10px` pill declarations with `var(--ch-font-xs)` (=11px, +1px) (#15); snap 26 `gap: 6px` to `var(--ch-space-1)`=4px (tight chip clusters/icon-to-text at tab-strip, goal/report chips, summary-run meta, menu items) and `var(--ch-space-2)`=8px (meta/button/list/header rows), converting the two-value `gap: 6px 8px` summary header to `var(--ch-space-1) var(--ch-space-2)` (#16); snap radii to the 5/7 scale — `3px` chips → `var(--ch-radius-sm)`, `6px` cards/pills and `8px` lightbox corners (including per-corner `8px 8px 0 0` / `0 0 8px 8px`) → `var(--ch-radius-md)`; the 2px sharp indicator left as-is per spec (#17). Brand-hex pill colors (#2b) intentionally excluded (depends on App.vue `--ch-agent-*` token follow-up). ~70 one-line CSS substitutions + 2 deletions; template+script byte-identical; postcss trap avoided.

- style(ui): App.vue shell bundle — brand tokens, hairline shadows, mode-bar motion (audit findings #2/#11/#14b). Define theme-invariant `--ch-agent-{claude,codex,origin,autonomy}-{bg,fg}` brand tokens in base `:root` (definitions only; consumer wiring deferred to Avatar Task 3/AWV #2b). Add themed `--ch-shadow-hairline` (dark: `0 1px 3px rgba(0,0,0,0.4)`; light: `0 1px 3px rgba(15,23,42,0.16)`) and reference it on `.theme-switch-thumb`; remove the redundant `.mode-button.active` `0 1px 2px` shadow (selected state already carried by `surface-selected` bg + `accent-ring-strong` border). Add `--ch-motion-drawer: 180ms cubic-bezier(0.2,0,0,1)` and unify `.app-mode-bar` so max-height/padding/border-color/opacity/transform all animate on the same drawer timing (the previous `opacity 160ms ease` odd-one-out is collapsed). CSS-only; template/script byte-identical.

- docs(ui): restructure audit next-3 for strict single-file dispatch (v7) — per reviewer feedback on v6, rebuild Section 5 so each of the three bounded tasks touches exactly one owning file with no cross-file leakage and no hidden cross-task dependencies: **Task 1 (App.vue only)** defines `--ch-agent-claude/codex/origin/autonomy-{bg,fg}` brand tokens in `:root`, resolves hairline shadows at 819/860, and consolidates mode-bar motion at 783 — explicitly NO consumer edits in this task. **Task 2 (AgentWorkspaceView only)** is a mechanical bulk-substitution bundle using only tokens that already exist (lightbox shadow → `--ch-shadow-dialog`, drop card-hover translateY+shadow, snap 9 off-token motion sites to `--ch-motion-fast/standard`, bulk-replace 25 `font-size:10px` → `--ch-font-xs`, snap 26 `gap:6px` → space tokens, snap scattered radii) and deliberately EXCLUDES #2b brand-pill wiring so it has zero dependency on Task 1 and can start concurrently. **Task 3 (AgentAvatar.vue only, sequenced after Task 1)** is a 4-line mechanical substitution wiring claude/codex hex at 150-157 to the new `--ch-agent-*` tokens. The AWV brand-pill wiring (#2b) and TerminalView xterm-padding snap (#10) move to Section 5b follow-ups so Section 5 stays at exactly three bounded tasks. Counts updated to reflect `6c79f45` (MarkdownContent prose-code radii, resolving #20 — was previously classified as INTENTIONAL) which shipped between v6 and v7 — now 13 RESOLVED via 8 commits (only #21 remains as documented intentional deviation), 9 findings in next-3, 2 follow-ups in 5b. Read-only doc; zero source edits across all review cycles.

- style(ui): snap `MarkdownContent.vue` inline-code + pre code-block radii to the 5/7 scale (audit finding #20) — inline `code` `border-radius: 4px` → `var(--ch-radius-sm)`=5px, `pre` block `border-radius: 6px` → `var(--ch-radius-md)`=7px; remove the now-stale `/* keep literal */` trailing comments on both rules and delete the two corresponding exception bullets in the file's doc-comment block so the remaining off-scale list stays accurate. CSS-only; template+script byte-identical; 1px corner change consistent with the EnvPresetManager radius pass (5eef835).

- docs(ui): finalize `docs/working-logs/2026-07-10-minimalist-ui-audit.md` (v6, post-ship refresh) — 13 of 26 audit findings shipped during review via 7 parallel-worker commits (`ce5b139`, `0f02b39`, `601c3cf`, `cde4352`, `d79afe2`, `5eef835`, `430d277`), covering LoginView CTA, LoadingButton focus ring, MobileControls motion, AgentAvatar palette/gradients/radii, TerminalPane fw500, EnvPresetManager weight/radii, and App.vue fw700; all marked RESOLVED with post-fix line refs. App.vue and AgentWorkspaceView.vue no longer deferred (prefetch `dfb26f3` + board-payload-slim `ff55553` both landed). MarkdownContent prose-code radii and LayoutSelector 2px tile reclassified as documented intentional deviations. Rebuild "Suggested Next 3" around remaining 11 actionable findings file-disjointly: Task 1 App.vue shell (hairline shadows + mode-bar motion + `--ch-agent-*` brand tokens), Task 2 AgentWorkspaceView board bundle (brand-pills/shadows/motion/type/radii — 7 findings), Task 3 TerminalView xterm 6px padding snap. Read-only doc; no source edits.
