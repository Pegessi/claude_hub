...
The in-flight tasks that originally gated edits to `App.vue` and `AgentWorkspaceView.vue` (prefetch and board-payload-slim) have both landed. In addition, seven of the originally-audited cosmetic fixes were picked up by parallel workers during the review cycle and are now on develop:
...
| `cde4352` | AgentAvatar palette tokenize + gradient flatten + radii snap | #6, #7, #8, #22 (partial on #2) |
...
Two items have explicit "keep literal" / "differs; keep literal" comments placed by prior token-pass workers and are treated as **documented intentional deviations**, not action items:

- #20 (MarkdownContent prose-code inline/block radii at 4/6px — a deliberate prose-code aesthetic)
- #21 (LayoutSelector 2px mini-tile radius — sharp affordance, awareness/no-op)

No source files are currently deferred. The remaining actionable items are consolidated into three file-disjoint bounded tasks in Section 5.
...
| 10 | **Injected xterm padding `6px` off the 4px rhythm** — JS-injected CSS sets `.xterm { padding: 6px !important }` (TerminalView.vue:947), between `space-1=4px` and `space-2=8px`. | `components/TerminalView.vue` | `TerminalView.vue:947` | low | S | Snap to `padding: 8px !important` (`var(--ch-space-2)` recommended — aligns with TerminalGridView gutter and is closer to current 6px visual weight than 4px). Single character change in a JS string; test by launching a terminal and verifying no canvas clipping/scrollbar overlap. Task 3. |
| 11 | **Hardcoded hairline shadows in App.vue shell not on palette** — `.mode-button.active` `box-shadow: 0 1px 2px rgba(15,23,42,0.08)` (819); floating FAB `0 1px 3px rgba(15,23,42,0.16)` (860). Hardcoded slate shadows, not themed for dark/light, not mapped to any `--ch-shadow-*` token. | `App.vue` | `App.vue:819, 860` | med | S | Remove (rely on `border-color` change) or add a themed `--ch-shadow-hairline` token. Bundle into App shell pass (Task 1). |
| 12 | **`box-shadow: 0 12px 48px rgba(0,0,0,0.45)` hardcoded on AWV lightbox** — close to `--ch-shadow-dialog` (0 24px 80px / 0.45) but hand-tuned and unthemed. | `components/AgentWorkspaceView.vue` | `AgentWorkspaceView.vue:9162` | med | S | Use `var(--ch-shadow-dialog)`; unifies elevation across overlays. Bundle into AWV board pass (Task 2). |
| 13 | **Card-hover large elevation adds visual noise on the board** — `box-shadow: 0 8px 24px var(--ch-shadow-color-soft)` plus `transform: translateY(-1px)` (6552-6553) on `.agent-status-card:hover`; minimalist aesthetic prefers `border-color`/`background` change only. | `components/AgentWorkspaceView.vue` | `AgentWorkspaceView.vue:6552-6553` | med | S | Drop hover shadow and `translateY(-1px)`; rely on `border-color: var(--ch-color-border-hover)` and `background: var(--ch-color-surface-raised)` only. Bundle into AWV board pass (Task 2). |
...
| 20 | **`MarkdownContent.vue` inline-code `border-radius: 4px` and block `border-radius: 6px`** — explicit `--ch-radius-sm=5px differs; keep literal` / `--ch-radius-md=7px differs; keep literal` comments (124-125, 193, 204). Prior token-pass worker intentionally kept prose-code radii slightly tighter than UI chrome as a deliberate prose-code aesthetic. Documented intentional deviation. | `components/MarkdownContent.vue` | `MarkdownContent.vue:124-125, 193, 204` | low (INTENTIONAL) | — | Leave as-is. |
...
**Counts (develop @ 430d277):**

- **RESOLVED / shipped:** 13 findings (#1, #3, #4, #6, #7, #8, #9, #14, #18, #19, #22, #23; #4 cross-listed motion+focus for history) via 7 commits.
- **INTENTIONAL / no-op:** 2 findings (#20, #21).
- **Actionable (in next-3):** 11 findings across 3 single-file tasks.
  - Task 1 (App.vue shell): #2 brand tokens, #11 hairline shadows, #14b mode-bar motion
  - Task 2 (AWV board): #2b, #12, #13, #14c, #15, #16, #17
  - Task 3 (TerminalView xterm padding): #10
- **Awareness (no dedicated task):** #5 LoadingButton spinner easing.
...
These are the **three highest-value, smallest-blast-radius, file-disjoint** follow-ups available right now against develop @ `430d277`. Each is scoped to a single owning file so a worker can finish in one small diff, and all three can run concurrently without merge conflicts. All originally-recommended tasks (AgentAvatar Task 1, MobileControls Task 2, EnvPresetManager Task 3) plus the TerminalPane fw500 standalone shipped in real time during review via parallel workers (`cde4352`, `601c3cf`, `5eef835`, `d79afe2`, `430d277`); the three tasks below are rebuilt from the remaining actionable work.

### Task 1 — `App.vue` shell bundle — hairline shadows + mode-bar motion + brand tokens
**File:** `frontend/src/App.vue`
**Scope:** `<style>` block (~15 lines across 3 sites plus 4-8 new token declarations). (1) Resolve the two hardcoded hairline shadows — remove `.mode-button.active` `box-shadow: 0 1px 2px rgba(15,23,42,0.08)` (819) in favor of the existing `border-color: var(--ch-color-accent-ring-strong)` + `background: var(--ch-color-surface-selected)`, and either remove or introduce a themed `--ch-shadow-hairline` token for the floating FAB `0 1px 3px rgba(15,23,42,0.16)` (860 — finding #11); (2) consolidate the quintuple-property `.app-mode-bar` transition (783) so `opacity` uses `var(--ch-motion-standard)` (180ms) instead of `160ms ease`, matching the other four properties' `200ms cubic-bezier` → `var(--ch-motion-drawer, 180ms cubic-bezier(.2,0,0,1))` (finding #14b); (3) add brand-token custom properties in `:root` (between existing terminal palette at 579 and closing `}` at 580) — `--ch-agent-claude-bg: #f1eee5; --ch-agent-claude-fg: #d97757; --ch-agent-codex-bg: #000; --ch-agent-codex-fg: #fff;` plus `--ch-agent-origin-bg/fg` (violet) and `--ch-agent-autonomy-bg/fg` (teal) for AWV pills in Task 2 — then reference the claude/codex pair in `AgentAvatar.vue:150-157` (finding #2).
**Why first:** App.vue is the design-token source-of-truth; fixing #11/#14b removes the last App-shell token drifts, and defining `--ch-agent-*` brand tokens unblocks the AWV brand-pill task (#2b / Task 2). All three edits are cosmetic scoped-style changes with zero behavioral impact; #1 (fw700) already shipped. Estimated diff: ~15 lines + 4-8 new token declarations; purely cosmetic. Covers findings #2 (App half), #11, #14b.

### Task 2 — `AgentWorkspaceView.vue` board bundle — brand pills + shadows + motion + type + radii
**File:** `frontend/src/components/AgentWorkspaceView.vue`
**Scope:** `<style>` block (~50-70 lines; bulk mechanical substitution). Depends on Task 1 landing the `--ch-agent-*` tokens. (1) Wire 12 brand-hex status-pill sites to tokens (Claude `#d97757`→`var(--ch-agent-claude-fg)`, Codex `#10a37f`/terminal `#7ee787`→appropriate `--ch-agent-*`/`--ch-terminal-*`, violet `#c4b5fd`/`rgba(139,92,246,…)`→`--ch-agent-origin-*`, teal `#5eead4`/`rgba(20,184,166,…)`→`--ch-agent-autonomy-*`) (finding #2b); (2) replace hardcoded lightbox `box-shadow: 0 12px 48px rgba(0,0,0,0.45)` (9162) with `var(--ch-shadow-dialog)` (finding #12); (3) drop `.agent-status-card:hover` `box-shadow: 0 8px 24px …` + `transform: translateY(-1px)` (6552-6553), leaving `border-color`/`background` changes only (finding #13); (4) snap all scattered off-token motion durations to tokens — `0.12s`/`0.08s`/`0.15s`/`0.1s` decimal shorthand at 6871/6891/6937/6958-6962/7130/7978/8022/8073 → `var(--ch-motion-fast)` (120ms), `opacity 240ms ease` at 7396 → `var(--ch-motion-standard)` (180ms); two `700ms linear infinite` spinners and the correct `180ms cubic-bezier` toast-in stay as-is (finding #14c); (5) bulk-replace 25 `font-size: 10px` pills → `var(--ch-font-xs)` (11px) (finding #15); (6) bulk-snap `gap: 6px` chip rhythm (26 sites) to `var(--ch-space-1)=4px` or `var(--ch-space-2)=8px` per use (finding #16); (7) snap scattered off-scale radii — 6px cards/pills (7231/8527/8633/10628) → `var(--ch-radius-md)`=7px, 3px chips (10800/10892) → `var(--ch-radius-sm)`=5px, 8px caps (9085/9096/9103) → `var(--ch-radius-md)`=7px, genuinely sharp 2px indicators (9629) stay (finding #17).
**Why second:** AWV is the largest single surface (board/cards/modals); once Task 1 lands the brand tokens, this is mechanical bulk substitution with zero design judgment. Biggest visual "all at once" polish win remaining; consolidates seven residual findings in one file. Estimated diff: ~50-70 lines of one-line substitutions spread across the file; purely cosmetic. Covers findings #2b, #12, #13, #14c, #15, #16, #17.

### Task 3 — `components/TerminalView.vue` — snap xterm padding to 8px (4px rhythm)
**File:** `frontend/src/components/TerminalView.vue`
**Scope:** JS-injected CSS string (~1 character at 947). Snap the injected `.xterm { padding: 6px !important }` (line 947, inside the JS string that builds xterm theme CSS) to `padding: 8px !important` (`var(--ch-space-2)`) — aligns with the TerminalGridView gutter and is closer to the current 6px visual weight than the tighter 4px alternative.
**Why third:** smallest remaining non-awareness fix (~1 character in a JS string); resolves the last spacing-rhythm finding outside the AWV bundle; cosmetic only; fully file-disjoint from Tasks 1 and 2. Flagged as standalone low-priority in prior versions because terminal chrome is sensitive to padding changes — test by launching a terminal session and verifying the xterm canvas has no clipping or scroll-bar overlap after the change. Covers finding #10.

## 5b. Standalone Awareness Items (not in a bounded task)

Not part of the "next 3" — single-line awareness items that do not warrant a dedicated bounded task:

- **`LoadingButton.vue:71` spinner `700ms linear infinite`** (finding #5): mechanical linear spinner is arguably intentional; leave unless/until a future motion polish pass.
- **`EnvPresetManager.vue:541` `.btn` `transition: background-color 0.2s`**: 200ms vs `--ch-motion-standard`=180ms; ships adjacent to the already-merged #9/#19 fix (`5eef835`) — flagging for awareness but not a new finding this cycle; any worker touching EnvPresetManager for other reasons can snap this to `var(--ch-motion-standard)` as a drive-by.
- **Documented intentional deviations (no action):** #20 MarkdownContent prose-code radii 4/6px (explicit "differs; keep literal" comment — deliberate prose-code aesthetic by prior token-pass worker); #21 LayoutSelector 2px tile radius (deliberately sharp affordance, awareness/no-op).
...
**Conclusion:** no workspace lessons applied directly to this read-only aesthetic audit. Thirteen of the originally 26 findings shipped during the six-cycle review via seven parallel-worker commits (`ce5b139`, `0f02b39`, `601c3cf`, `cde4352`, `d79afe2`, `5eef835`, `430d277`); the remaining 11 actionable items are consolidated into three file-disjoint bounded tasks above (App shell, AWV board, TerminalView xterm padding).
