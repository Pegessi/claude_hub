#!/usr/bin/env python3
"""E2E: workspace board lazy-load — initial 15 tasks, load-more reveals 16+."""
...
def browser_lazyload_evidence(
    workspace_id: str,
    sixteenth_title: str,
    expected_initial_titles: list[str],
    expected_after_titles: list[str],
) -> dict[str, Any]:
    ...
    with sync_playwright() as playwright:
        ...
        sixteenth_visible_before = page.locator(".task-card h3", has_text=sixteenth_title).count()

        load_more = page.locator(".board-history-load-more")
        assert load_more.count() == 1, page.content()
        load_more.click()
        page.wait_for_function(
            "(title) => document.querySelectorAll('.task-card h3').length >= 16 "
            "&& Array.from(document.querySelectorAll('.task-card h3')).some("
        ...
        sixteenth_visible_after = page.locator(".task-card h3", has_text=sixteenth_title).count()
    ...
    return {
        ...
        "sixteenth_title_expected": sixteenth_title,
        "expected_initial_titles": expected_initial_titles,
        "expected_after_titles_prefix": expected_after_titles,
...
def main() -> None:
    ...
    try:
        ...
        for index in range(FIXTURE_COUNT):
            task_status, task = http(
                "POST",
                f"/api/workspaces/{workspace_id}/tasks",
                {
                    "title": f"Lazy fixture {index:02d}",
                    "prompt": f"Fixture task {index}",
                },
            )
            assert task_status == 201, task
            task_ids.append(task["id"])
            if index > 0:
                http(
                    "PATCH",
                    f"/api/workspaces/tasks/{task_ids[index - 1]}",
                    {"status": "done"},
                )
            time.sleep(0.01)

        evidence["fixture_task_ids"] = task_ids
        ...
        assert len(full_board["tasks"]) == FIXTURE_COUNT
        assert full_board.get("tasks_pagination") is not None
        evidence["full_board_sorted_titles"] = [
            task.get("title", "") for task in full_board["tasks"]
        ]
        evidence["full_board_sorted_ids"] = [task["id"] for task in full_board["tasks"]]

        sixteenth_task = full_board["tasks"][15]
        sixteenth_title = sixteenth_task["title"]
        expected_initial_titles = [task.get("title", "") for task in full_board["tasks"][:15]]
        expected_after_titles = [task.get("title", "") for task in full_board["tasks"][:16]]
        evidence["sixteenth_task_id"] = sixteenth_task["id"]
        evidence["sixteenth_title"] = sixteenth_title
        ...
        assert len(initial_board["tasks"]) == 15
        assert initial_board["tasks_pagination"]["total_count"] == FIXTURE_COUNT
        assert [task["id"] for task in initial_board["tasks"]] == [
            task["id"] for task in full_board["tasks"][:15]
        ]
        ...
        assert len(page2_board["tasks"]) == FIXTURE_COUNT - 15
        expected_page2_ids = [task["id"] for task in full_board["tasks"][15:]]
        assert evidence["page2_api_task_ids"] == expected_page2_ids
        assert evidence["page2_api_task_ids"][0] == sixteenth_task["id"]

        frontend_proc = start_frontend_preview()
        browser = browser_lazyload_evidence(
            workspace_id,
            sixteenth_title,
            expected_initial_titles,
            expected_after_titles,
        )
        evidence["browser"] = browser

        assert browser["initial_dom_task_cards"] == 15, browser
        ...
        for response in browser["initial_board_responses_with_limit_15"]:
            ...
            assert response["tasks_length"] == 15, response
        ...
        assert cursor_response["task_ids"][0] == sixteenth_task["id"], cursor_response
        ...
        assert browser["after_load_more_dom_task_cards"] >= 16, browser
