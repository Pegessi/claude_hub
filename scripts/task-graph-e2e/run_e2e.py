    ...
    *,
    ...
    for event in after_events:
        ...
        if call_id in original_call_ids:
            assert call_id not in seen_original, (
...
def _reviewer_report(workspace_id: str, child_id: str, reviewer_session_id: str) -> dict | None:
    ...
    terminal = [
        r
        for r in owned
        if str(r.get("state") or "").lower() in _TERMINAL_REPORT_STATES
    ]
...
def main() -> int:
    ...
    try:
        ...
        assert evidence["parent_compat_run_id_before_reload"] == compat_run_id_for_task(parent_model)
        assert evidence["child_compat_run_id_before_reload"] == compat_run_id_for_task(child_model)
        ...
        parent_after_ack = task_cli("get", parent_id, "--workspace-id", workspace_id, "--no-reports")
